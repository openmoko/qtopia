...
PerformancePrivate = {
    ...
    testStartupTime_qpe: function()
    {
        ...
        var format = "%t %n(%p): %s";
        setSetting("Trolltech", "Log",  "QtopiaServer",   "Enabled", 1);
...
Performance = {
    ...
    */
    ...
    /*
        ...
        appName is the application's binary name (e.g. "addressbook").
        displayName is the application's display name for selecting from
        the launcher menu (e.g. "Applications/Contacts").

        appName may be "qpe", in which case displayName can be omitted.
    */
    testStartupTime: function(appName, displayName)
    {
        waitForQtopiaStart();
        if (appName == "qpe") {
        ...
        // Turn on the desired log categories and formatting
        var format = "%t %n(%p): %s";
        var categories = [ "QtUitest", "QtopiaServer", "Quicklauncher", "ApplicationLauncher", "UI" ];
        for (var i = 0; i < categories.length; ++i) {
            setSetting("Trolltech", "Log", categories[i], "Enabled", 1);
        }
        setSetting("Trolltech", "Log2", "MessageHandler", "Format",  format);
        LogPrivate.format = format;
        ...
        for (var i = 0; i < 1 + REPEAT; ++i) {
            ...
            try {
                ...
                fail("Could not find one or more expected log messages!\n" + e);
            }
