...
*/
    sendMessage: function( to, subject, msg, frm, replyto, cc, bcc )
    {
        ...
        var mailArguments = [ "-s "+subjectLine, "-r "+senderAddress ];
        if( cc ) mailArguments.push("-c "+cc);
        if( bcc ) mailArguments.push("-b "+bcc);
        mailArguments.push(to);
        var reply = runProcess( "mail", mailArguments, msg );
        return reply;
    },
