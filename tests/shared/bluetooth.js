...
/*
    \usecase setBluetooth - handles setting the various valus of the bluetooth UI - can be called in two ways: predetermined arguments and named arguments
        predetermined arguments - setBluetooth( state, visiblity, timeout, name ) - with null used when an argument is not to change but a later one is
                                - setBluetooth( true, null, null, "foobar") - set bluetooth on + changeing name to string supplied.
...
*/
    ...
    setBluetooth: function( state, visiblity, timeout, name )
    {
        ...
        select( "Settings/Bluetooth", launcherMenu() );
        waitForTitle( "Bluetooth" );
