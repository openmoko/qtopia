...
UnixMakefileGenerator::writeSubdirs(QTextStream &t, bool direct)
{
	...
	for( it.toFirst(); it.current(); ++it) {
	    ...
	    t << "if $(MAKE) -n -f " << (*it)->makefile << " qmake_all >/dev/null 2>&1; then "
	      << "$(MAKE) -f " << (*it)->makefile << " qmake_all" << "; fi; )";
	}
	t << endl;
	...
	for( it.toFirst(); it.current(); ++it) {
	    ...
	    if(!(*it)->directory.isEmpty())
		t << "[ -d " << (*it)->directory << " ] && cd " << (*it)->directory << " ; ";
	    t << "if $(MAKE) -n -f " << (*it)->makefile << " $@ >/dev/null 2>&1; then "
	      << "$(MAKE) -f " << (*it)->makefile << " $@" << "; fi; )";
	}
	t << endl;
	...
	for( it.toFirst(); it.current(); ++it) {
	    ...
	    if(!(*it)->directory.isEmpty())
		t << "[ -d " << (*it)->directory << " ] && cd " << (*it)->directory << " ; ";
	    t << "if $(MAKE) -n -f " << (*it)->makefile << " uninstall >/dev/null 2>&1; then "
	      << "$(MAKE) -f " << (*it)->makefile << " uninstall" << "; fi; )";
	}
	t << endl;
	...
	for( it.toFirst(); it.current(); ++it) {
	    ...
	    if(!(*it)->directory.isEmpty())
		t << "[ -d " << (*it)->directory << " ] && cd " << (*it)->directory << " ; ";
	    t << "if $(MAKE) -n -f " << (*it)->makefile << " install >/dev/null 2>&1; then "
	      << "$(MAKE) -f " << (*it)->makefile << " install" << "; fi; )";
	}
	t << endl;
	...
	for( it.toFirst(); it.current(); ++it) {
		...
		t << "[ -d " << (*it)->directory << " ] && cd " << (*it)->directory << " ; ";
	    t << "if $(MAKE) -n -f " << (*it)->makefile << " $@ >/dev/null 2>&1; then "
	      << "$(MAKE) -f " << (*it)->makefile << " $@; $(DEL_FILE) " << (*it)->makefile << "; fi; )";
	}
	t << endl << endl;
    }
