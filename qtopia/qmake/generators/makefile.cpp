...
MakefileGenerator::init()
{
    ...
    QString paths[] = { QString("SOURCES"), QString("FORMS"), QString("YACCSOURCES"), QString("INCLUDEPATH"),
			    ...
			    QString("LEXSOURCES"), QString("QMAKE_INTERNAL_INCLUDED_FILES"),
			    QString("TRANSLATABLES"), QString::null };
	...
	if(!tmp.isEmpty()) {
		...
		if(QFile::exists(wild)) { //real file
		    ...
		    QString cmd =  QString(fi.isDir() ? "-$(INSTALL_DIR)" : "-$(INSTALL_FILE)") + " \"" +
				   ...
				   "\" \"" + root + dst +
#ifdef Q_WS_WIN
				   QString(fi.isDir() ? fi.fileName().prepend(Option::dir_sep) : "") +
#endif
				   "\"\n";
		    target += cmd;
