    ...
    : QWidget( parent, name )
{
	...
	it[i] = menu->insertItem(IRController::stateDescription((IRController::State)i));
    menu->setItemChecked(irc->state(),TRUE);
	...
	if ( ic.pixmap().isNull() )
	    menu->insertItem(n,protobase+i);
	else
	    menu->insertItem(ic,n,protobase+i);
    }
    menu->setItemChecked(protobase+irc->currentProtocol(),TRUE);
    ...
    connect(irc, SIGNAL(stateChanged(State)), this, SLOT(checkState()));
    setFixedWidth( 0 );
}
...
void IRReceiverApplet::checkState()
{
    for (int i=0; i<=(int)IRController::LastState; i++) {
	bool y = irc->state()==i;
...
void IRReceiverApplet::activate(int choice)
{
    ...
    if ( choice >= protobase ) {
	if ( !irc->setProtocol(choice-protobase) )
	    choice = -1;
	for (int i=protobase; i-protobase<irc->protocolCount(); i++)
	...
	for (int i=0; i<=(int)IRController::LastState; i++) {
	    ...
	    if ( y )
		irc->setState((IRController::State)i);
	}
    }
}
