...
QStringList Simple8Impl::names() const
{
    QStringList r;
    int i=0;
    do {
	r.append(unicodevalues[i].cs);
    } while ( unicodevalues[i++].mib != LAST_MIB );
    return r;
}

QTextCodec *Simple8Impl::createForName( const QString &name )
{
    int i=0;
    do {
	if ( name == unicodevalues[i].cs )
	    return new QSimpleTextCodec(i);
    } while ( unicodevalues[i++].mib != LAST_MIB );
    return 0;
}

    
QValueList<int> Simple8Impl::mibEnums() const
{
    QValueList<int> r;
    int i=0;
    do {
        r.append(unicodevalues[i].mib);
    } while ( unicodevalues[i++].mib != LAST_MIB );
    return r;
}

QTextCodec *Simple8Impl::createForMib( int mib )
{
    int i=0;
    do {
	if ( mib == unicodevalues[i].mib )
	    return new QSimpleTextCodec(i);
    } while ( unicodevalues[i++].mib != LAST_MIB );
    return 0;
}
