...
FormSimple::FormSimple(QWidget *parent) :QWidget(parent) {
    ...
    bool portrait = d->width() < d->height();
    if ( portrait ) {
        grid = new QGridLayout(this, 5, 4, 0 , 3, "grid");
        grid->addMultiCellWidget(siw, 1, 4, 0, 3);
    } else {
        grid = new QGridLayout(this, 4, 5, 0, 3,  "grid");
...
#ifdef QTOPIA_PHONE
    PBMPlus->setFocusPolicy(NoFocus);
    PBMC->setFocusPolicy(NoFocus);
    PBMR->setFocusPolicy(NoFocus);
    PBCE->setFocusPolicy(NoFocus);
#endif
