...
{
	printf( "SipTransaction: Incoming Response\n" );
...
//---
			...
			case Sip::NOTIFY:
				...
				if( message->getMethod() == Sip::NOTIFY ) {
					...
					if (idx != -1) {
						m = m.mid( idx + 11 );
						parent->setPresenceStatus(stateText( m.left( m.find('"') ) ));
					} else {
						m = message->messageBody();
						idx = m.find("status status=");
						if (idx != -1) {
							m = m.mid( idx + 15 );
							parent->setPresenceStatus(m.left( m.find('"') ));
						} else {
...
void SipTransaction::incomingRequestRetransmission( SipMessage *message )
{
	SdpMessage optsdp;
	QString p;
	printf( "SipTransaction: Incoming Request Retransmission\n" );
	...
	if( ( laststatus.getCode() < 200 ) && ( direction == LocalRequest ) ) {
		...
		if( requestmessage->getMethod() != Sip::INVITE ) {
			if( diff > requestmessage->lastTimeTick() ) {
				...
				printf( "SipTransaction: Retransmit %d (%d)\n",
