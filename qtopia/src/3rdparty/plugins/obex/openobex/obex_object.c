...
obex_object_t *obex_object_new(void)
{
	...
	object = (obex_object_t*)malloc(sizeof(obex_object_t));
	if (object == NULL)
		return(NULL);
	...
	obex_headerdata_t hv, uint32_t hv_size, unsigned int flags)
{
	...
	element = (struct obex_header_element*)malloc(sizeof(struct obex_header_element));
	if(element == NULL)
		return -1;
				...
				uint8_t *source, unsigned int len)
{
	...
	if(hi == OBEX_HDR_BODY_END)	{
		...
		if( (element = (struct obex_header_element*)malloc(sizeof(struct obex_header_element)) ) ) {
			memset(element, 0, sizeof(struct obex_header_element));
