...
sub parse_pro_file
{
    ...
    open PRO, "$proFile" or die "Can't open $proFile\n";
    my @data = <PRO>;
    close PRO;
    while ( @data ) {
        $_ = shift(@data);
...
sub qtopia_project($$$)
{
    ...
    my $keywords = "$depotpath/src/build/keywords.prf";
    ...
    # This logic should match qtopia_project() in src/build/functions.prf
...
sub implicit_deps($$$)
{
    ...
    # This logic should match src/build/implcit_deps.pri
    ...
    if ( $$CONFIG{qtopia} ) {
        if ( opt("qtopiabase") && !$$CONFIG{no_qtopiabase} ) {
    ...
    if ( $$CONFIG{qtopiadesktop} && !$$PROJECT_TYPE{core} && $$CONFIG{plugin} ) {
