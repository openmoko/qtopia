...
###
        ...
        if ( proj.isProperty("QMAKE_PRL_LIBS") )
            libs.append(proj.property("QMAKE_PRL_LIBS").value());
    }
}
...
function qt_resolveQt(qt)
{
    ...
    // No trigger for testlib so use the qtuitest check instead
    else if ( qt == "testlib" ) {
        requires("contains(CONFIG,qtuitest)");
        obj.qlib = "QtTest";
    }
...
function qt_get_libs(qtdir,sdkdir)
{
    ...
    // Qt doesn't tell us if this is on or not (use the Qt Extended switch instead)
    if (project.config("qtuitest"))
        ret.push(sdkdir+"/lib/libQtTest"+suffix);
