        ...
        for ( propname in data ) {
            ...
            if ( type == "existingFile" || type == "existingFiles" ) {
                ...
                if ( typeof(value) == "string" ) {
                    // Package the single value into an array (so the can use the same code below)
                    var tmp = new Array();
                    tmp.push(value);
                    value = tmp;
                }
                ...
                for ( vi in value ) {
                    var v = value[vi];
                    // This only does files
                    var files = project.files(v);
                    ...
                    // This only does directories
                    files = project.paths(v);
                    if ( files == null ) files = new Array();
                    for ( ii in files ) {
                        if ( files[ii].exists() ) {
                            f.push(files[ii]);
