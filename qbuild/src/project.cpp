...
{
    RUN_TRACE
        qWarning() << "Including file" << fileName;

    if (!fileName.isValid())
        return false;

    m_includedFiles << fileName.fsPath();

    PreprocessorToken error;
    QString errorMessage;
    Block *block = QMakeParser::parseFile(fileName.fsPath(), &error, &errorMessage);
    if (!block) {
        ...
        qOutput().nospace()
            ...
            << "ERROR: Error parsing file " << fileName.fsPath() << " at line "
