...
*/
Rule *SchedulerRule::rule() const
{
    return _state?_state->rule:0;
}
    ...
    if (rule->state->executeState == RuleEngineState::Queued) {
        ...
        if (!rule->state->depRules.isEmpty()) {
            if (rule->flags & Rule::SerialRule) {
                if (options.debug & Options::RuleExec)
                    qTraceOutput() << "QBuild(" << m_id << "):     Waiting on" << rule->state->depRules.first().rule()->ruleName();
    ...
    if (rule->state->executeState == RuleEngineState::Waiting) {
        ...
        if (rule->flags & Rule::SerialRule &&
           rule->state->serialProgression < rule->state->depRules.count()) {
            ...
            Rule *depRule =
                rule->state->depRules.at(rule->state->serialProgression - 1).rule();

            if (depRule->state->executeState == RuleEngineState::DoneFailed) {
            ...
            if (options.debug & Options::RuleExec)
                qTraceOutput() << "QBuild(" << m_id << "):     Waiting on" << rule->state->depRules.at(rule->state->serialProgression).rule()->ruleName();
        ...
        for (int ii = 0; ii < rule->state->depRules.count(); ++ii) {
            RuleEngineState *state = rule->state->depRules.at(ii).state();
            Rule *depRule = rule->state->depRules.at(ii).rule();
