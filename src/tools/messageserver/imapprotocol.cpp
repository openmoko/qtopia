        ...
        if (str == QString::null) {
            ...
            if (fetchResponsePattern.indexIn(str) == 0) {
                if (fetchResponsePattern.cap(1).compare("FETCH", Qt::CaseInsensitive) == 0) {
                    QString uid = extractUid(str, _name);
                    uint msgSize = extractSize(str);
                    ...
                    createMail( msg.toLatin1(), uid, msgSize, flags );
                    return true;
                } else {
                    // Untagged response - skip it
                    str = d->next();
                    moreData = !str.isEmpty();
                }
    ...
    do {
        ...
        if (str == QString::null) {
            ...
            if (fetchResponsePattern.indexIn(str) == 0) {
                ...
                qLog(IMAP) << qPrintable("fetch all failed: " + str);
            } else {
                emit nonexistentMessage(messageUid(_name, fetchUid), Client::Removed);
                return true;
            }
        }
    } while (moreData);
