    ...
    : QObject(parent)
{
    ...
    connect(QMailStore::instance(),SIGNAL(messageRemovalRecordsAdded(QMailMessageIdList)),
            this, SLOT(deletedMessages(QMailMessageIdList)));
}
...
void EmailHandler::nonexistentMessage(const QString& uid, Client::DefunctReason reason)
{
    if (const Client* client = static_cast<const Client*>(sender())) {
        ...
        if (message.id().isValid()) {
            ...
            QMailStore::instance()->purgeMessageRemovalRecord(accountId, uid);
        }
    }
...
void EmailHandler::deletedMessages(const QMailMessageIdList& mailList)
{
    QMailMessageRemovalRecordList messagesRemoved = QMailStore::instance()->messageRemovalRecords(mailList);
    QMailMessageIdList recordsToPurge;

    foreach (const QMailMessageRemovalRecord& removalRecord, messagesRemoved ) {
        if (!removalRecord.serverUid().isEmpty()) {
            // Delete from client storage, where appropriate
            if (Client *client = clientForAccount(removalRecord.parentAccountId(), true)) {
                if (client->hasDeleteImmediately()) {
                    client->deleteImmediately(removalRecord.serverUid());
                    recordsToPurge.append(removalRecord.id());
                }
            }
        } else {
            //if the serveruid is empty, the message was created locally, so purge anyway
            recordsToPurge.append(removalRecord.id());
        }
    }

    if (!recordsToPurge.isEmpty())
        QMailStore::instance()->purgeMessageRemovalRecords(recordsToPurge);
}
