...
bool ImapClient::setNextDeleted()
{
    if (_config.canDeleteMail()) {
        if (!_removedUids.isEmpty()) {
            ...
            QMailStore::instance()->purgeMessageRemovalRecord(accountId,msgUidl);
...
QMailFolderId ImapClient::getMessageMailbox(const QString &serverUid)
{
    QString mailboxName = QtMail::messageMailbox(serverUid);

    QMailAccount account(accountId);
    QMailFolderId folderId(account.getMailbox(mailboxName));
    if (folderId.isValid()) {
        if (account.serverUids(folderId).contains(serverUid))
            return folderId;
    }
...
void ImapClient::fetchNextMail()
{
    ...
    if (selectionItr != selectionMap.end()) {
        ...
        mailboxId = getMessageMailbox(serverUid);
        while ((!mailboxId.isValid()) && (selectionItr != selectionMap.end())) {
            qLog(Messaging) << "ImapClient::fetchNextMail: cant find uid" << serverUid << "in box" << QtMail::messageMailbox(serverUid);
...
{     //set some other parameters
    ...
    if(selectionMap.contains(retrieveUid))
        mail.setId(selectionMap.value(retrieveUid));
