...
*/
void QSystemTest::putFile( const QString &srcFile, const QString &destFile )
{
    QFile f(srcFile);
    if (!f.open(QIODevice::ReadOnly))
        QFAIL( "Couldn't open '" + srcFile + "'" );

    putData(f.readAll(), destFile);
}
...
*/
void QSystemTest::putData( const QByteArray &data, const QString &destFile )
{
...
class TestProcess : public QProcess
{
    ...
    private slots:
        void output()
        {
            QString text = readAllStandardOutput();
            while (text.endsWith("\n")) text.chop(1);
            foreach (QString const& line, text.split('\n') ) {
                // Filter out noise
                int i=0;
                bool is_noise = false;
                ...
                if (!is_noise) QDebug(QtDebugMsg) << qPrintable(line);
            }
        }

        void error()
        {
            QString text = readAllStandardError();
            while (text.endsWith("\n")) text.chop(1);
            foreach (QString const& line, text.split('\n') )
                if (!line.isEmpty()) {
                    // Filter out noise
    ...
    protected:
        virtual void setupChildProcess()
        {
            ...
            static QString basePath = QSettings("Trolltech", "QtUitest").value("paths/image", QTUITEST_IMAGE_PATH).toString();
            static QString preload = QString("%1/lib/libqtuitest.so:%1/qtslave/libqtslave.so:%1/loader/libqtuitestloader.so").arg(basePath);
            static QString widgets = QString("%1/qtwidgets").arg(basePath);

            setenv("LD_PRELOAD", qPrintable(preload), 1);
            setenv("QTUITEST_WIDGETS_PATH", qPrintable(widgets), 1);
#endif
        }
};
...
void QSystemTest::startApplication( const QString &application, int timeout, StartApplicationFlags flags )
{
    ...
    QProcess* proc = new TestProcess(this);
...
/*!
    ...
    \sa setDateTime(), setTimeSynchronization(), incrementDateTime()
*/
...
/*!
    ...
    \sa getDateTime(), setTimeSynchronization(), incrementDateTime()
*/
...
/*!
    ...
    \sa getDateTime(), setDateTime(), incrementDateTime()
*/
...
/*!
    Increments/decrements the current time on the device with \a milliseconds.
    This call is more accurate and efficient than i.e. 'getting the current time + calculating an incremented time + setting the new time'.

    \sa setTimeSynchronization(), getDateTime(), setDateTime()
*/
void QSystemTest::incrementDateTime( int /*milliseconds*/ )
{
    QFAIL("QSystemTest::incrementDateTime not implemented yet");
}
...
/*!
    Returns the currently set Visual Response Time. This time is used in QtUiTest to decide wether the User
