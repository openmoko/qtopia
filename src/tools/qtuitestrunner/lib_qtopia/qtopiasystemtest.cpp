...
/*!
    ...
    Once initialized:
    \list
    \o the device time will be synchronized according to \a mode.
    \o the Date format will be set to 'D/M/Y'.
    \o the Time format will be set to 24 hour.
    \endlist
*/
void QtopiaSystemTest::waitForQtopiaStart( int timeout, QSystemTest::TimeSynchronizationMode mode )
{
    ...
    if (ok) {
        int offset = setTimeSynchronization(mode);
        if (offset != 0) qWarning( "Device time adjusted by %d s.", offset );
...
void QtopiaSystemTest::setPerformanceTest(bool on, bool forceRestart)
{
    ...
    if (on) {
        ...
        if (!runsOnDevice()) {
            ...
            if (queryFailed()) return;
            needs_restart = d->ensureEnvironment("QTOPIA_PERFTEST_LAUNCH", "$(date +%T.%N)") || needs_restart;
            if (queryFailed()) return;
        ...
        if (!runsOnDevice()) {
            ...
            if (queryFailed()) return;
            needs_restart = d->ensureEnvironment("QTOPIA_PERFTEST_LAUNCH", QString()) || needs_restart;
            if (queryFailed()) return;
...
void QtopiaSystemTest::processCommandLine( int &argc, char *argv[] )
{
    ...
    /*
        Stream the output of Qtopia to a log file.  Although we could read directly from Qtopia's
        output in the desktop case, this simulates behavior on a device where logs must be
        retrieved through some device-specific mechanism (usually just reading a certain file).
    */
    d->qtopia_script = "/bin/sh -c \""
"export QTOPIA_LOG=$(mktemp -t qtuitestrunner_qtopialog.XXXXXX); "
"trap 'rm -f $QTOPIA_LOG' EXIT; " +
launchPath + "/runqtopia -test -runmode systemtesthelper > $QTOPIA_LOG 2>&1; "
"QTOPIA_EXITCODE=$?; "
"rm -f $QTOPIA_LOG; "
"( exit $QTOPIA_EXITCODE; );\""
    ;
