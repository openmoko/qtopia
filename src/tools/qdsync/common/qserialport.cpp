...
bool QSerialPort::open( OpenMode mode )
{
    ...
    // Wait for the IO Thread to get started
    LOG() << "Waiting for the IO Thread";
    HANDLE handles[2] = { d->io->writeSem, d->io->quitSem };
    LONG ret = WaitForMultipleObjects( 2, handles, true, INFINITE );
    Q_ASSERT( ret == WAIT_OBJECT_0 );
    LOG() << "IO Thread up";
#endif
