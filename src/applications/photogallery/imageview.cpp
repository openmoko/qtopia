...
void ImageView::setAsBackgroundImage()
{
    ...
    if (QFileSystem::fromFileName(m_content.fileName()).isRemovable()) {
    ...
    if (!fileName.isNull()) {
        ...
        cfg.beginGroup(QLatin1String("HomeScreen"));
        cfg.setValue(QLatin1String("HomeScreenPicture"), fileName);

        QtopiaChannel::send(QLatin1String("QPE/System"), QLatin1String("updateHomeScreenImage()"));
    }
