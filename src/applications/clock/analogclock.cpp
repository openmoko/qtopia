...
void AnalogClock::drawContents( QPainter *pp )
{
    ...
    if (!face.isEmpty()) {
        QPixmap pix = QPixmap(":image/"+face);
        if (!pix.isNull()) {
            facePm = pix.scaled(size, size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
        }
    }
