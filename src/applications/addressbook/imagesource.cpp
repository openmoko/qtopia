	...
	*/
	...
	for( QStringList::Iterator it = apps.begin() ; it != apps.end() ; ++it )
	{
		...
		ValueServiceServer src(cfg.readEntry("Name").stripWhiteSpace(), 
				QCString("QPE/Application/")+(*it).latin1(), 
								actionName, 
...
ValueServiceServer::ValueServiceServer( const QString &n, const QCString &c, const QCString &a, const QPixmap &p )
    : mName( n ), mChannel( c ), mAction( a ), mPix( p )
{
}
