...
AbEditor::AbEditor( const PimContact &entry, const QValueList<int> *newOrdered,
...
void AbEditor::init()
{
    ...
    l = new QLabel( tr("Pronunciation"), container );
    gl->addWidget( l, 2, 0 );
    pronunciationEdit = new QLineEdit( container );
    gl->addWidget( pronunciationEdit, 2, 1 );

    l = new QLabel( tr("Categories"), container );
    gl->addWidget( l, 3, 0 );

    cmbCat = new CategorySelect( container );
    gl->addWidget( cmbCat, 3, 1 );
    ...
    gl->addWidget( genderCombo, slOrdered->count() + 4, 1 );
...
void AbEditor::setEntry( const PimContact &entry )
{
    ...
    pronunciationEdit->setText( ent.pronunciation() );
    cmbCat->setCategories( ent.categories(), "Address Book", tr("Contacts") );
...
void AbEditor::saveEntry()
{
    ...
    ent.setPronunciation( pronunciationEdit->text() );
...
static bool isSlightlyValidEmail(const QString& e)
{
    int s = e.find('"');
    if (s>0) s = e.find('"',s);
    else s = 0;
...
void parseEmailTo( const QString &strDefaultEmail,
		   const QString &strOtherEmail, QString &strBack )
{
    // create a comma delimited set of emails...
    ...
    where = 0;
    while ( where > -1 ) {
	// We just look at the string enough to know that "blah" <foo> is 1 email address
	start = where;
	do {
	    where = strOtherEmail.find( ' ', where + 1 );
	} while ( !isSlightlyValidEmail(strTmp = strOtherEmail.mid(start, where - start).stripWhiteSpace())
	    && where > 0 ); // (uint)(where-start) is big if where<0
	if ( foundDefault || strTmp != strDefaultEmail ) {
	    if ( isSlightlyValidEmail( strTmp ) ) {
		strBack.append( ", " );
