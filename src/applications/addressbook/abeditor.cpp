...
AbstractField::AbstractField( QWidget *parent , const char *name )
    : QWidget( parent, name ), mModified( FALSE ), mHaveFieldsPrevChanged( FALSE ), detailEditor(0)
{
}
...
{
    mModified = mHaveFieldsPrevChanged = b;
}
...
{
    if( mHaveFieldsPrevChanged ) // have the fields changed previously?
    {
	mModified = TRUE;
    }
    else // allow for the initial setting of fields
    {
	mHaveFieldsPrevChanged = TRUE;
    }
}
...
void AbEditor::editPhoto()
{
	...
	photoPB->setPixmap( mContactImage );
    }
    delete iface;
}
...
void AbEditor::catCheckBoxChanged( bool  b  )
{
	...
	if( curCats.contains( bcatid ) ) 
	{
	    QArray<int> tmpCats( curCats.copy() );
	    const int ns =  tmpCats.count()-1;
	    curCats.resize( ns );
	    for( int i = 0 ; i < ns ; ++i )
		if( tmpCats[i] != bcatid )
		    curCats[i] = tmpCats[i];
...
void AbEditor::categorySelectChanged( int catid )
{
    Categories c;
    bool loadOK = c.load( categoryFileName() );
    int bcid = c.id( "addressbook", "_Business" );
    if( loadOK && bcid && bcid == catid )
...
void AbEditor::accept()
{
...
#ifdef QTOPIA_PHONE
	//use tmp so that if the user says no to the following question, the original contact is unaffected
	if( !tmp.customField("SIM_CONTACT").isEmpty() && (AbUtil::numbersForSimCount(tmp) == 0) && !hasNonSimFields() )
	{
	    // sim contact with no numbers for sim and no other data worth saving (only a name is possible)
	    int r = QMessageBox::warning( this, tr("Remove SIM Contact"), tr("<qt>Your changes to the SIM contact '%1' will cause it to be removed entirely."
			" Are you sure you want to do this?</qt>").arg(ent.fileAs()), QMessageBox::Yes, QMessageBox::No | QMessageBox::Default );
	    if( r == QMessageBox::No )
		return;
	}
#endif
	...
	if( mContactImage.isNull() )
	{
	    ...
	    {
		//the user actually selected no image/removed the image
		ent.removeCustomField( "photofile" );
		QFile pFile( baseDirStr+pFileName );
		if( pFile.exists() )
		    pFile.remove();	
	    }
	    else
	...
	else
	{
	    if( pFileName.isEmpty() )
	    {
		//new image for this contact
		pFileName = AbEditor::createContactImage( mContactImage );
		if( !pFileName.isEmpty() )
		    ent.setCustomField( "photofile", pFileName );
	    }
	    else
	    {
		//modified image for this contact
		if( !mContactImage.save( baseDirStr+pFileName, "JPEG", 50 ) )
		    qWarning("Unable to save contact image '%s'", (baseDirStr+pFileName).latin1());
	    }
	}
#endif
...
//creates a new contact image from fn
QString AbEditor::createContactImage( const QString &fn )
{
    QPixmap copy( fn, "jpeg" );
    if( copy.isNull() )
	return QString::null;

    return AbEditor::createContactImage( copy );
}
