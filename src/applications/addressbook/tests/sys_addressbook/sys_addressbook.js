...
*/
    ...
    sending_a_contacts_details_via_email: function(contactdata, text)
    {
        if( !testcase.userAccountCreated ){
            Messages.createEmailAccount( Accounts.getAccount(1) );
            testcase.userAccountCreated = true;
        ...
        expectMessageBox("Sending","Sending: Email", "OK") {
            enter(contactdata.email, "To", NoCommit);
            select( "Send", softMenu() );
        }

        waitForTitle( "Contact Details" );

        // Todo: verify that email is truly sent
    },
...
*/
    ...
    {
        var hasCamera = false;
        // FIXME: detect if the device has a camera at runtime
        if (!hasCamera) {
            skip( "No camera in device" );
        }
        ...
        prompt( "Verify that the 'Pictures' is launched with the photograph displayed in edit mode." );
        // 12. Change the brightness of the image then return to the contacts application.
        prompt( "Change the brightness of the image then return to the contacts application." );
        // 13. Verify that the edited image is now displayed in the 'Contact Photo' dialog.
        prompt( "Verify that the edited image is now displayed in the 'Contact Photo' dialog." );
        // 14. Select the 'Done' option to return to the 'New Contact' dialog, then again to return to the contacts list.
        select( "Back", softMenu() );
        waitForTitle( "New Contact" );
        select( "Back", softMenu() );
        waitForTitle( "Contacts" );

        // 15. Verify that the image selected is viewable from the contacts list and in the details view.
        prompt( "Verify that the image selected is viewable from the contacts list and in the details view." );

        // 16. Select the contact again, then 'Edit' from the context menu.
        select(contactdata.name);
        select( "Select", softMenu() );
        waitForTitle( "Contact Details" );
        select("Edit",optionsMenu());
        waitForTitle( "Edit Contact" );
    },
...
*/
    ...
    {
        putFile( baseDataPath() + contactdata.imagefilename, documentsPath() + contactdata.imagefilename);
        Content.rescan();
...
*/
    ...
    {
        Performance.testStartupTime("addressbook", "Applications/Contacts");
    },
