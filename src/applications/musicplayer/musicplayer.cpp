    ...
    void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
    {
        ...
        if( viewType == MusicPlayerPrivate::vtAlbumView || viewType == MusicPlayerPrivate::vtPlaylistView || viewType == MusicPlayerPrivate::vtGenreView ) {
            if (option.state & QStyle::State_Selected)
            {
                ...
                painter->drawText(option.rect, Qt::AlignLeft|Qt::AlignVCenter, index.model()->data(index, Qt::DisplayRole).toString());
            }
            else
            {
                QColor bg = Qt::black;
            ...
            if (option.state & QStyle::State_Selected)
            {
                ...
                tmpfont.setBold(true);
                painter->setFont(tmpfont);
                int offset=option.rect.height()/10;
                painter->drawText(QRect(option.rect.x(), option.rect.y(), option.rect.width()-option.rect.height()-(offset*2), option.rect.height()), Qt::AlignLeft|Qt::AlignVCenter, index.model()->data(index, Qt::DisplayRole).toString());
                QRect arrowRect(option.rect.x()+option.rect.width()-option.rect.height(), option.rect.y()+offset, option.rect.height()-offset, option.rect.height()-(offset*2));
                QIcon arrow;
            ...
            else
            {
                ...
                QRect initialLetterRect(option.rect.x(), option.rect.y(), painter->fontMetrics().width(text,1), option.rect.height());
                QRect restOfStringRect(option.rect.x()+initialLetterRect.width(), option.rect.y(), option.rect.width(), option.rect.height());
