...
#include <qvaluespace.h>
    ...
    /*
The expression framework is basically a simple compiler and software CPU system.
The compiler operates in 3 passes. First it tokenizes the input and reports any lexical errors (unexpected characters, unexpected end of input etc).
Second it parses the input into a tree structure, enforcing expression syntax and precedence , and also performs semantic checking at this stage.
The third pass does code generation. Tokenization is represented by the class ExpressionTokenizer, parsing by ExpressionParser and semantic checking by
ExpressionSemanticAnalyser, and the last stage of code generation is represented by ExpressionCodeGenerator. The intermeddiate tree representation is
represented by the class ExpressionParserNode, and is passed from the ExpressionParser to the ExpressionCodeGenerator. ExpressionCodeGenerator, generates
a list of instructions, executed from left to the right, and a list of data operands, which the instructions operate on, which is referenced left to right.
This execution is performed by ExpressionMachine class. When the ExpressionMachine encounters a Load instruction, it takes data from the left of the data
list and pushes it on the stack. Different instructions expect different types of parameters to be on the stack. Left to right associativity is represented
in the stack as bottom to top - that is, data that is to the left of some other data in the expression is deeper in the stack. This means for a binary operation,
the top value is the rvalue.  Static coercion occurs on constants in the expression, eg for 3.0 * 3, 3 will be be coerced to double during the semantic checking phase.
In the case of variable data('Terms'), return types of expressions are checkecd to ensure runtime coercion can take place correctly. For example in the expression
2.0 * ("20" . ".5") the semantic checker sees that the lhs of the operation * is a double and the rhs is a string, the result of the . string operation.
The string is not known until runtime, and because static coercion is not possible,  but one double and one string are still valid operands to the * operator,
the string value is coerced to a double at runtime. Failure to convert at runtime results in a runtime semantic error. */
...
#ifdef QTOPIAIL
    QReference<QAbstractExpressionEvaluatorTerm> ref;
#else
    QString vskey;
#endif
};
...
#ifdef QTOPIAIL
    QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >  terms() const;
...
#ifdef QTOPIAIL
    QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >  m_terms;
    int m_termCount;
#endif
...
#ifdef QTOPIAIL
            , const QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >& terms
#endif
            ) ;
...
private:
    ...
    QVariant m_result;
#ifdef QTOPIAIL
    QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >  m_t;
#endif
    bool m_errorflag;
};
...
bool ExpressionTokenizer::tokenize() {
    ...
    while( !m_idevice.atEnd() ) {
        ...
        qWarning("Expression Input Error2");
            return false;
        }
            ...
            default:
            {
                if( isspace( c ) ) {
                    ...
                    do {
                        if( !m_idevice.getChar( &c ) ) {
                            // XXX error: input error
        qWarning("Expression Input Error3");
                            return false;
                        }
                        if( isspace( c ) )
                            break;
...
bool ExpressionTokenizer::tokenizeNumber( char c ) {
    ...
    do {
        ...
        if( !m_idevice.getChar( &c ) ) {
            // XXX error: input error
qWarning("Expression Input Error4");
            return false;
        }
    } while( isdigit( c ) || (!seenDot && c == '.'));
    if( num.endsWith('.') ) {
        // XXX error: double must have decimal component
...
bool ExpressionTokenizer::tokenizeString() {
    ...
    while(true) {
        if( m_idevice.atEnd() ) {
            // XXX error: unexpected end of input
qWarning("Expression Unexpected end of input");
            return false;
        }
        if( !m_idevice.getChar( &c ) ) {
            // XXX error: input error
qWarning("Expression Input Error6");
            return false;
        }
...
/* Public Methods */
/* ExpressionParserNode Ctors */
ExpressionParserNode::ExpressionParserNode( ExpressionParser* parser, const ExpressionToken& t ) : token(t), returnType(ExpressionToken::Null), leftChild(0), rightChild(0) {
...
bool ExpressionSemanticAnalyser::checkSemantics( ExpressionParserNode* node ) {
    ...
    if( node->token.type != ExpressionToken::Assign ) {
        /* If we have children that are terms, ensure we can read from them */
        if( leftChild->token.type == ExpressionToken::Term ) {
            Q_ASSERT( !leftChild->ref.isNull() );
            if( !leftChild->ref->canRead() ) {
                qWarning("Expression Cannot read from term '%s', invalid use in this expression.", leftChild->token.data.constData());
                return false;
            }
        }
        if( rightChild != 0 && rightChild->token.type == ExpressionToken::Term ) {
            Q_ASSERT( !rightChild->ref.isNull() );
            if( !rightChild->ref->canRead() ) {
                qWarning("Expression Cannot read from term '%s', invalid use in this expression.", rightChild->token.data.constData());
                return false;
            }
        }
    }
    ...
    if( leftChild != 0 && rightChild == 0 ) {
       switch( node->token.type ) {
        ...
        switch( node->token.type ) {
            ...
            {
#ifdef QTOPIAIL
                // if the left is a term that is not writeable, semantic error
                if( leftChild->token.type != ExpressionToken::Term ) {
                    qWarning("Expression Invalid l-value in assignment, expected a term");
                    return false;
                } else {
                    Q_ASSERT(!leftChild->ref.isNull());
                    if( !leftChild->ref->canWrite() ) {
                        qWarning("Expression Cannot assign value to term '%s' that is read-only", node->token.data.constData());
                        return false;
                    }
                }
                // XXX FIXME can this term accept the value being assigned to it
#endif
                return true;
            }
        default:
...
void ExpressionSemanticAnalyser::setReturnType(ExpressionParserNode* node) {
    ...
    if( !leftChild && !rightChild  ) {
        ...
        switch( node->token.type ) {
            ...
            {
#ifdef QTOPIAIL
                Q_ASSERT(!node->ref.isNull());
                switch( node->ref->dataType() ) {
                    case QAbstractExpressionEvaluatorTerm::String:
                        node->returnType = ExpressionToken::String;
                        break;
                    case QAbstractExpressionEvaluatorTerm::Double:
                        node->returnType = ExpressionToken::Double;
                        break;
                    case QAbstractExpressionEvaluatorTerm::Integer:
                        node->returnType = ExpressionToken::Integer;
                        break;
                    case QAbstractExpressionEvaluatorTerm::Bool:
...
/* Public Methods */
/* ExpressionCodeGenerator Ctors */
ExpressionCodeGenerator::ExpressionCodeGenerator( ExpressionParserNode* node ) : m_root(node)
#ifdef QTOPIAIL
    , m_termCount(0)
#endif
    , m_useFixedPoint(false)
{
    Q_ASSERT(m_root != 0);
}
...
#ifdef QTOPIAIL
QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >  ExpressionCodeGenerator::terms() const
{
    return m_terms;
}
#endif
...
void ExpressionCodeGenerator::generateDataCode( ExpressionParserNode* data ) {
    ...
    if( data->token.type == ExpressionToken::Term ) {
        ...
        Q_ASSERT( !data->ref.isNull() );
        m_terms.insert(m_termCount, data->ref);
#else
...
#ifndef QT_NO_DEBUG
    if( node->token.type != ExpressionToken::Not && node->token.type != ExpressionToken::Minus) {
        Q_ASSERT(node->leftChild != 0 && node->rightChild != 0);
    } else {
        Q_ASSERT((node->leftChild != 0 && node->rightChild == 0) || (node->token.type == ExpressionToken::Minus && node->rightChild != 0));
    }
    if( node->rightChild != 0 )
        Q_ASSERT(node->leftChild->returnType == node->rightChild->returnType);
#endif
...
ExpressionParserNode* ExpressionParser::parsePrimary()
{
    ...
    switch( m_tokenizer->current().type ) {
        ...
        {
#ifdef QTOPIAIL
            // resolve an unknown term implementation for this term
            QList<QAnonymousReference> termHandlers = QtopiaIL::createInstanceX( "ExpressionTerm" );
            // XXX IL : need to be able to query this stuff without creating instances
            // FIXME make expression term interface handle sets instead of individual terms!
            QReference<QAbstractExpressionEvaluatorTerm> ref;
            for( QList<QAnonymousReference>::Iterator it = termHandlers.begin() ; it != termHandlers.end() ; ++it ) {
                ref = qt_cast<QAbstractExpressionEvaluatorTerm>(*it);
                Q_ASSERT( !ref.isNull() );
                if( ref->canHandle( QString(m_tokenizer->current().data.constData()) ) )
                    break;
                ref = QReference<QAbstractExpressionEvaluatorTerm>();
            }
            if( ref.isNull() ) {
                qWarning("Expression Error, unable to find a QAbstractExpressionEvaluatorTerm implementation that can handle the term '%s'. Maybe not a valid term?", m_tokenizer->current().data.constData());
                return 0;
            }
            ref->setTerm( QString(m_tokenizer->current().data.constData()) );
#endif
...
The expression evaluator has the ability to use values from the Qtopia valuespace in its expressions.
...
bool QExpressionEvaluator::evaluate()
{
    ...
    if( !d->machine->execute() ) {
        // XXX error: machine runtime error
...
bool QExpressionEvaluator::setExpression( const QByteArray& expr )
{
    ...
    if( d->expressionData != expr ) {
        ...
        if( node == 0 ) {
            // XXX error: Unable to parse the expression
...
//  dumps generated instructions and data, for debugging
...
#ifdef QTOPIAIL
                QHash<int, QReference<QAbstractExpressionEvaluatorTerm> > terms = ss->terms();
                for( QHash<int, QReference<QAbstractExpressionEvaluatorTerm> >::Iterator it = terms.begin () ;
                                                                                                    it != terms.end() ; ++it )
                    qSignal(*it,termChanged(QReference<QAbstractExpressionEvaluatorTerm>)) += qSlot(this, termChanged(const QReference<QAbstractExpressionEvaluatorTerm>&));
#endif
                ...
                // XXX error: semantic error in the expression
...
#ifdef QTOPIAIL
void QExpressionEvaluator::termChanged( const QReference<QAbstractExpressionEvaluatorTerm>& )
{
    emit termsChanged();
}
#endif
