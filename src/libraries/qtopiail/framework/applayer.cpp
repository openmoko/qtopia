...
/* not used
static int vsmemcmp(const QByteArray & s1, const QByteArray & s2)
{
    return vsmemcmp(s1.constData(), s1.size(), s2.constData(), s2.size());
}
*/
...
private:
    ...
    void dumpRecur(unsigned short, unsigned int depth);
...
/*!
  Print out memory consumption information and the current contents of
  the application layer.
  */
void FixedMemoryTree::dump()
{
    pool->dumpStats();

    dumpRecur(ROOT_VERSION_ENTRY, 0);
}
...
/*!
  \internal
  Recursive implementation of FixedMemoryTree::dump()
 */
void FixedMemoryTree::dumpRecur(unsigned short node, unsigned int depth)
{
    Node * const me = getNode(node);
    Q_ASSERT(me);

    QByteArray ba(depth * 4, ' ');
    QByteArray name(me->name, me->nameLen);

    qWarning("%s%s (version: %d data length: %d watch count: %d)",
             ba.constData(), name.constData(),
             versionTable()->entries[node].version, me->dataCount,
             me->watchCount);
    unsigned short * subNodes = (unsigned short *)fromPtr(me->subNodePtr);
    for(unsigned short ii = 0; ii < me->subNodes; ++ii)
        dumpRecur(subNodes[ii], depth + 1);
}
                                           ...
                                           unsigned int len, bool * match)
{
    ...
    // XXX Linear search for now - binary search soon
                              ...
                              unsigned int dataLen)
{
    ...
    if(0 == me->dataCount ||
       (1 == me->dataCount &&
        ((NodeDatum *)me->dataBegin())->owner == owner)) {
...
#ifdef QVALUESPACE_DEBUG_VERBOSE
    layer->dump();
#endif
    updateStats();
    lock->unlock();
...
{
    // XXX - if objects are deleted etc. in here we have a problem
