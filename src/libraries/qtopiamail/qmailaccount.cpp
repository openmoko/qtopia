...
*/
QStringList QMailAccount::deletedMessages() const
{
...
/*!
    Return the list of messages removed from the mailbox identified by \a boxId.
*/
QStringList QMailAccount::deletedMessages(const QMailFolderId &boxId) const
{
    QStringList serverUidList;
...
/*!
  Returns the list of message server UIDs for this account.
*/

QStringList QMailAccount::serverUids() const
{
    return serverUids(messagesKey() | trashKey());
}

/*!
    Returns the list of message server UIDs for this account, that are located in the folder identified by \a boxId.
*/

QStringList QMailAccount::serverUids(const QMailFolderId &boxId) const
{
    ...
    return serverUids(messagesKey(boxId) | trashKey(boxId));
}

/*!
    Returns the list of message server UIDs located in the folder identified by 
    \a boxId, whose status field yields \a set when logically ANDed with \a messageStatusFilter.
*/

QStringList QMailAccount::serverUids(const QMailFolderId &boxId, quint64 messageStatusFilter, bool set) const
{
    QMailMessageKey statusKey(QMailMessageKey::Status, messageStatusFilter, QMailDataComparator::Includes);
    return serverUids((messagesKey(boxId) | trashKey(boxId)) & (set ? statusKey : ~statusKey));
}
...
/*!
  Returns the storage key used to query for messages in this folder.
*/
QMailMessageKey QMailAccount::messagesKey() const
{
    QMailMessageKey messageAccountKey(QMailMessageKey::ParentAccountId,id());

    // All accounts except IMAP accounts store messages in the inbox folder
    QMailMessageKey messageParentFolderKey;
    if (accountType() == IMAP) {
        // Messages can be in any folder associated with this account
        QMailFolderKey accountFolders(QMailFolderKey::ParentAccountId, id());
        messageParentFolderKey = QMailMessageKey(QMailMessageKey::ParentFolderId, accountFolders);
    } else {
        messageParentFolderKey = QMailMessageKey(QMailMessageKey::ParentFolderId, QMailFolderId(QMailFolder::InboxFolder));
    }

    return messageAccountKey & messageParentFolderKey;
}

/*!
  Returns the storage key used to query for messages in this mailbox folder.
*/
QMailMessageKey QMailAccount::messagesKey(const QMailFolderId &boxId) const
{
    QMailMessageKey mailboxKey(QMailMessageKey::ParentFolderId, boxId);
    return mailboxKey;
}

/*!
  Returns the storage key used to query for messages moved from this mailbox folder to the Trash folder.
*/
QMailMessageKey QMailAccount::trashKey(const QMailFolderId &boxId) const
{
    QMailMessageKey trashFolderKey(QMailMessageKey::ParentFolderId, QMailFolderId(QMailFolder::TrashFolder));
    QMailMessageKey mailboxKey(QMailMessageKey::PreviousParentFolderId, boxId);

    return trashFolderKey & mailboxKey;
}
...
QMailMessageKey QMailAccount::trashKey() const
{
    ...
    return trashFolderKey & messageAccountKey;
}
