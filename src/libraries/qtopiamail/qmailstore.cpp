...
/*!
    ...
    that have been removed from local storage. Records are retrieved for messages whose ids are 
    contained in \a ids.  
    This information is primarily used for synchronization of local changes to remote message storage 
    ...
    an error or no data.
*/
QMailMessageRemovalRecordList QMailStore::messageRemovalRecords(const QMailMessageIdList& ids) const
{
    if (ids.isEmpty())
        return QMailMessageRemovalRecordList();

    // Resolve from overloaded member functions:
    AttemptResult (QMailStore::*func)(const QMailMessageIdList&, QMailMessageRemovalRecordList*, MailStoreReadLock&) const = &QMailStore::attemptMessageRemovalRecords;

    QMailMessageRemovalRecordList removalRecords;
    repeatedly<ReadAccess>(bind(func, this, cref(ids), &removalRecords), "messageRemovalRecords(ids)");
...
{
    // Resolve from overloaded member functions:
    AttemptResult (QMailStore::*func)(const QMailAccountId&, const QString&, QMailMessageRemovalRecordList*, MailStoreReadLock&) const = &QMailStore::attemptMessageRemovalRecords;

    QMailMessageRemovalRecordList removalRecords;
    repeatedly<ReadAccess>(bind(func, this, cref(accountId), cref(fromMailbox), &removalRecords), "messageRemovalRecords(accountId, fromMailbox)");
    return removalRecords;
}

/*!
    Erases message deletion records in the list \a ids from the store. 

    Returns \c true if the operation completed successfully, \c false otherwise. 
*/
bool QMailStore::purgeMessageRemovalRecords(const QMailMessageIdList& ids)
{
    return repeatedly<WriteAccess>(bind(&QMailStore::attemptPurgeMessageRemovalRecords, this, cref(ids)), "purgeMessageRemovalRecords");
}

/*!
    Erases message deletion records from the account with id \a accountId and 
    server uid \a serverUid.

    Returns \c true if the operation completed successfully, \c false otherwise. 
*/
bool QMailStore::purgeMessageRemovalRecord(const QMailAccountId& accountId, const QString& serverUid)
{
    return repeatedly<WriteAccess>(bind(&QMailStore::attemptPurgeMessageRemovalRecord, this, cref(accountId), cref(serverUid)), "purgeMessageRemovalRecords");
}
...
QMailStore::AttemptResult QMailStore::attemptRemoveAccounts(const QMailAccountKey &key, MailStoreTransaction& t)
{
    ...
    QMailMessageIdList deletedRemovalRecords;
    QStringList expiredMailfiles;

    if (d->deleteAccounts(key, deletedAccounts, deletedFolders, deletedMessages, expiredMailfiles, deletedRemovalRecords)) {
        if (t.commit()) {
            //remove deleted objects from caches
            removeExpiredData(deletedMessages, expiredMailfiles, deletedFolders, deletedAccounts);

            //synchronize
            d->notifyMessageRemovalRecordsChange(QMailStorePrivate::Removed, deletedRemovalRecords);
            ...
            emit messageRemovalRecordsRemoved(deletedRemovalRecords);
...
QMailStore::AttemptResult QMailStore::attemptRemoveFolders(const QMailFolderKey &key, QMailStore::MessageRemovalOption option, MailStoreTransaction& t)
{
    ...
    QMailMessageIdList createdRemovalRecords;

    if (d->deleteFolders(key, option, deletedFolders, deletedMessages, expiredMailfiles, modifiedAccounts, createdRemovalRecords)) {
        if (t.commit()) {
            //remove deleted objects from caches
            ...
            if (!createdRemovalRecords.isEmpty())
                d->notifyMessageRemovalRecordsChange(QMailStorePrivate::Added, createdRemovalRecords);
            ...
            if (!createdRemovalRecords.isEmpty())
                emit messageRemovalRecordsAdded(createdRemovalRecords);
...
QMailStore::AttemptResult QMailStore::attemptRemoveMessages(const QMailMessageKey &key, QMailStore::MessageRemovalOption option, MailStoreTransaction& t)
{
    ...
    if (d->deleteMessages(key, option, deletedMessages, expiredMailfiles, modifiedAccounts, modifiedFolders, createdRemovalRecords)) {
        if (t.commit()) {
            //remove deleted objects from caches
...
QMailStore::AttemptResult QMailStore::attemptUpdateMessagesStatus(const QMailMessageKey &key, quint64 status, bool set, MailStoreTransaction& t)
{
    ...
    result = affectedByMessageIds(modifiedMessageIds, &modifiedFolderIds, &modifiedAccountIds);
    if (result != Success)
        return result;
                                                              ...
                                                              MailStoreReadLock&) const
{
    ...
    while (query.next())
        result->append(d->messageMetaData(query.record(), properties));

    return Success;
}

/*! \internal */
QMailStore::AttemptResult QMailStore::attemptMessageRemovalRecords(const QMailMessageIdList &ids, QMailMessageRemovalRecordList *result, MailStoreReadLock&) const
{
    QString sql("SELECT * FROM deletedmessages WHERE id IN ");
    sql += d->expandValueList(ids.count());
...
/*! \internal */
QMailStore::AttemptResult QMailStore::attemptPurgeMessageRemovalRecords(const QMailMessageIdList &removedMessageIds, MailStoreTransaction& t)
{
    // nothing to remove
    ...
    {
        //get the messages we can remove
        QSqlQuery query(d->simpleQuery("SELECT id FROM deletedmessages WHERE",
                                       QMailMessageKey(removedMessageIds),
                                       "purgeMessageRemovalRecords info query"));
        if (query.lastError().type() != QSqlError::NoError)
            return DatabaseFailure;
    ...
    if (!removalIds.isEmpty()) {
        QSqlQuery query(d->simpleQuery("DELETE FROM deletedmessages WHERE",
                                       ...
                                       "purgeMessageRemovalRecords delete query"));
        if (query.lastError().type() != QSqlError::NoError)
            return DatabaseFailure;
    }
    ...
    if (t.commit()) {
        if (!removalIds.isEmpty()) {
            d->notifyMessageRemovalRecordsChange(QMailStorePrivate::Removed,removalIds);
            emit messageRemovalRecordsRemoved(removalIds);
        }
...
/*! \internal */
QMailStore::AttemptResult QMailStore::attemptPurgeMessageRemovalRecord(const QMailAccountId &accountId, const QString &serverUid, MailStoreTransaction& t)
{
    QMailMessageIdList removalIds;

    {
        QSqlQuery query(d->simpleQuery("SELECT id FROM deletedmessages WHERE parentaccountid=? AND serveruid=?",
                                        QVariantList() << accountId.toULongLong() << serverUid,
...
QMailStore::AttemptResult QMailStore::attemptRestoreToPreviousFolder(const QMailMessageKey &key, MailStoreTransaction& t)
{
    ...
    {
        // Find the folders that are affected by this update
        QSqlQuery query(d->simpleQuery("SELECT parentfolderid, previousparentfolderid FROM mailmessages WHERE",
                                       key,
        ...
        while (query.next()) {
            folderIdSet.insert(d->extractValue<quint64>(query.value(0)));
...
void QMailStore::preloadHeaderCache(const QMailMessageId& id) const
{
    ...
    if(index != -1)
    {
        //preload based on result of last call to QMailStore::queryMessages
        int startIndex = index - QMailStorePrivate::lookAhead;
        if(startIndex < 0) startIndex = 0;

        int endIndex = index + QMailStorePrivate::lookAhead;
        if(endIndex > d->lastQueryMessageResult.count()) endIndex = d->lastQueryMessageResult.count();

        for (int i = startIndex ; i < endIndex; ++i)
            idBatch.append(d->lastQueryMessageResult.at(i));
    }
    else
    {
        //preload based on locality of id in storage
        quint64 idValue(id.toULongLong());
        for (int i = -QMailStorePrivate::lookAhead; i < QMailStorePrivate::lookAhead; ++i)
            idBatch.append(QMailMessageId(idValue + i));
    }
...
/*!
    \fn void QMailStore::messageRemovalRecordsAdded(const QMailMessageIdList& ids)

    Signal that is emitted when the QMailMessageRemovalRecords in the list \a ids are
    added to the store.
...
/*!
    \fn void QMailStore::messageRemovalRecordsRemoved(const QMailMessageIdList& ids)
