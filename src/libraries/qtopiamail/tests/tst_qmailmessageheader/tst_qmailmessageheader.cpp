...
void tst_QMailMessageHeaderField::decodedContent_data()
{
    ...
    input = "=?UTF-8?B?2LYg2akJ2qUg2rQ=?=; param=UTF-8'ar'%D8%B6%D9%A9%DA%A5%DA%B4%20%3Caddress%40domain%3E";
...
void tst_QMailMessageHeaderField::encodeWord_data()
{
    ...
    */
    ...
    QTest::newRow("ISO-8859-1 with encoding")
        << latin1Address
        << QByteArray("ISO-8859-1")
        << QByteArray("=?ISO-8859-1?Q?Joh=F1_D=F6e_=3Cjdoe=40machine=2Enet=3E?=");
    ...
    latin1Address = QString("\"Joh\361 D\366e\" <jdoe@machine.net>");

    QTest::newRow("ISO-8859-1 with quoted encoding")
        << latin1Address
        << QByteArray("ISO-8859-1")
        << QByteArray("=?ISO-8859-1?Q?=22Joh=F1_D=F6e=22_=3Cjdoe=40machine=2Enet=3E?=");
    ...
    QTest::newRow("UTF-8 with encoding")
        << unicodeAddress
        << QByteArray("UTF-8")
        << QByteArray("=?UTF-8?B?2LbZqdql2rQgPGFkZHJlc3NAZG9tYWluPg==?=");
    ...
    QTest::newRow("UTF-8 with language")
        << unicodeAddress
        << QByteArray("UTF-8*ar")
        << QByteArray("=?UTF-8*ar?B?2LbZqdql2rQgPGFkZHJlc3NAZG9tYWluPg==?=");

    unicodeAddress.append(unicodeAddress);
    QTest::newRow("Excessive length requires splitting")
        << unicodeAddress
        << QByteArray()
        << QByteArray("=?UTF-8?B?2LbZqdql2rQgPGFkZHJlc3NAZG9tYWluPti22anapdq0IDxhZGRyZXNzQGRv?= =?UTF-8?B?bWFpbj4=?=");
}
...
void tst_QMailMessageHeaderField::encodeParameter_data()
{
    ...
    QTest::newRow("Latin1 with encoding")
        ...
        << QByteArray("ISO-8859-1")
        << QByteArray("de")
        << QByteArray("ISO-8859-1'de'%22Joh%F1%20D%F6e%22%20%3Cjdoe%40machine%2Enet%3E");
    ...
    QTest::newRow("Latin1 with encoding overridden by language")
        ...
        << QByteArray("en_US")
        << QByteArray("ISO-8859-1'en_US'%22Joh%F1%20D%F6e%22%20%3Cjdoe%40machine%2Enet%3E");

    QTest::newRow("Latin1 without context")
        << latin1Address
        << QByteArray()
        << QByteArray()
        << QByteArray("ISO-8859-1''%22Joh%F1%20D%F6e%22%20%3Cjdoe%40machine%2Enet%3E");

    QTest::newRow("UTF-8")
        << unicodeAddress
        << QByteArray("UTF-8")
        << QByteArray("ar")
        << QByteArray("UTF-8'ar'%D8%B6%D9%A9%DA%A5%DA%B4%20%3Caddress%40domain%3E");

    QTest::newRow("UTF-8 without context")
        << unicodeAddress
        << QByteArray()
        << QByteArray()
        << QByteArray("UTF-8''%D8%B6%D9%A9%DA%A5%DA%B4%20%3Caddress%40domain%3E");
}
...
void tst_QMailMessageHeaderField::encodeContent_data()
{
    ...
    */
    ...
    QTest::newRow("ISO-8859-1 with encoding")
        << latin1Address
        << QByteArray("ISO-8859-1")
        << QByteArray("=?ISO-8859-1?Q?Joh=F1?= =?ISO-8859-1?Q?_D=F6e?= =?ISO-8859-1?Q?_=3Cjdoe=40machine=2Enet=3E?=");

    QTest::newRow("Deduced ISO-8859-1")
        << latin1Address
        << QByteArray()
        << QByteArray("=?ISO-8859-1?Q?Joh=F1?= =?ISO-8859-1?Q?_D=F6e?= <jdoe@machine.net>");
    ...
    QTest::newRow("ISO-8859-1 with quoted encoding")
        << latin1Address
        << QByteArray("ISO-8859-1")
        << QByteArray("=?ISO-8859-1?Q?=22Joh=F1_D=F6e=22?= =?ISO-8859-1?Q?_=3Cjdoe=40machine=2Enet=3E?=");
    ...
    QTest::newRow("UTF-8 with encoding")
        << unicodeAddress
        << QByteArray("UTF-8")
        << QByteArray("=?UTF-8?B?2LbZqdql2rQ=?= =?UTF-8?B?IDxhZGRyZXNzQGRvbWFpbj4=?=");

    QTest::newRow("Deduced UTF-8")
        << unicodeAddress
        << QByteArray()
        << QByteArray("=?UTF-8?B?2LbZqdql2rQ=?= <address@domain>");

    QTest::newRow("UTF-8 with language")
        << unicodeAddress
        << QByteArray("UTF-8*ar")
        << QByteArray("=?UTF-8*ar?B?2LbZqdql2rQ=?= =?UTF-8*ar?B?IDxhZGRyZXNzQGRvbWFpbj4=?=");
}
...
"\theader_field..." CRLF);
    ...
    QTest::newRow("Unbreakable token after breakable whitespace")
        ...
        << QByteArray("attachment; filename=/home/mvogt/build/qtopia/4.3/phone-debug/home/Documents/channell1.jpg")
        << QByteArray(
"Content-Disposition: attachment;" CRLF
" filename=/home/mvogt/build/qtopia/4.3/phone-debug/home/Documents/channell1.jp" CRLF
"\tg" CRLF);
}
