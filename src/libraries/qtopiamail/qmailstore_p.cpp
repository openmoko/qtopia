...
protected:
    ...
    RecordExtractorBase(const QSqlRecord &r) : record(r) {}
    ...
    { 
        if (record.isNull(field))
            ...
            return QMailStorePrivate::extractValue<ValueType>(record.value(field), defaultValue);
    }
...
class MessageRecord : public RecordExtractorBase<QMailMessageKey::Property>
{
public:
    MessageRecord(const QSqlRecord &r) 
        : RecordExtractorBase<QMailMessageKey::Property>(r) {}
...
public:
    ...
    QMailMessageId id() const { return QMailMessageId(value<quint64>("id")); }
                                        ...
                                        QMailMessageMetaData* metaData) const
{
    ...
    const QList<QMailMessageKey::Property> keys = messagePropertyMap().keys();

    // Use wrapper to extract data items
    const MessageRecord messageRecord(r);
                                                        ...
                                                        const QMailMessageKey::Properties& properties) const
{
    ...
    messageMetaData(r, properties, &metaData);
    return metaData;
}
                                                  ...
                                                  const QMailMessageKey::Properties& properties) const
{
    ...
    // Load the meta data items
    messageMetaData(r, properties, &newMessage);
...
QVariantList QMailStorePrivate::updateValues(const QMailMessageKey::Properties& properties, const QMailMessageMetaData& data)
{
    ...
    static const QList<QMailMessageKey::Property> keys = messagePropertyMap().keys();
    foreach (const QMailMessageKey::Property& p, keys)
    {
        switch(properties & p)
        {
            case QMailMessageKey::Id:
...
bool QMailStorePrivate::initialized() const
{
    return init;
}
...
bool QMailStorePrivate::updateMessageBody(const QString& fileName, const QMailMessage& data)
{
    QString filePath = messageFilePath(fileName);
                                       ...
                                       QMailMessageIdList& createdRemovalRecords)
{
...

    {
        ...
        while (infoQuery.next()) {
            ...
            if (option == QMailStore::CreateRemovalRecord) {
                ...
                removalIds.append(messageId.toULongLong());
                ...
                createdRemovalRecords.append(messageId);
            }
        }
    }
    ...
    if (createdRemovalRecords.count()) {
        // WARNING - QList::operator<<(QList) actually appends the list items to the object,
        // rather than insert the actual list!
        QSqlQuery insertRemovalQuery(simpleQuery("INSERT INTO deletedmessages (id,parentaccountid,serveruid,frommailbox) VALUES (?,?,?,?)",
                                                 QVariantList() << QVariant(removalIds) 
                                                                << QVariant(removalAccountIds)
                                      ...
                                      QMailMessageIdList& createdRemovalRecords)
{
    ...
    if (!deleteMessages(messagesKey, option, deletedMessages, expiredMailfiles, modifiedAccounts, modifiedFolders, createdRemovalRecords))
        return false;
                                       ...
                                       QMailMessageIdList& deletedRemovalRecords)
{
    ...
    {
        // Find the IDs of the removal records that we will delete - using a folder key is not
        // strictly correct, but will work because both tables name the relevant field 'parentaccountid'...
        QSqlQuery removalRecordQuery(simpleQuery("SELECT id FROM deletedmessages WHERE",
                                                 foldersKey,
                                                 "deleteAccounts removal record query"));
        if (removalRecordQuery.lastError().type() != QSqlError::NoError)
            return false;

        while (removalRecordQuery.next())
            deletedRemovalRecords.append(QMailMessageId(extractValue<quint64>(removalRecordQuery.value(0))));
    }
    ...
    if (!deleteFolders(foldersKey, option, deletedFolders, deletedMessages, expiredMailfiles, modifiedAccounts, createdRemovalRecords))
        return false;
