...
void QAlternateStack::start(QAlternateStackEntryPoint entry, const QVariant& data)
{
    ...
    d->alternateStack.ss_sp    = d->stackBuffer.data();
    d->alternateStack.ss_flags = 0;
    d->alternateStack.ss_size  = d->stackBuffer.size();
...
bool QAlternateStack::isCurrentStack() const
{
    ...
    // stack buffer.
    char dummy = 1;
    quint32 diff = &dummy - d->stackBuffer.constData();
    return (diff < (quint32)d->stackBuffer.size());
}
...
bool QAlternateStack::isAvailable()
{
    ...
    // Known to be broken on Greenphone.
    return false;
#else
    return true;
#endif
