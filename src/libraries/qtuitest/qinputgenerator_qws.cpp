...
void QInputGeneratorPrivate::keyEvent(Qt::Key key, Qt::KeyboardModifiers mod, KeyEventType type, bool autoRepeat)
{
    ...
    QWSServer::processKeyEvent(unicode, key, mod, (type != KeyRelease), false);

    if (type == KeyClick) {
        // Key press/release should not occur in same millisecond
        QtUiTest::wait(10);
        QWSServer::processKeyEvent(unicode, key, mod, false, false);
    }
...
void QInputGenerator::keyPress(Qt::Key key, Qt::KeyboardModifiers mod)
{ d->keyEvent(key, mod, d->KeyPress); }
...
void QInputGeneratorPrivate::mouseEvent(QPoint const& pos, Qt::MouseButtons state, MouseEventType type)
{
