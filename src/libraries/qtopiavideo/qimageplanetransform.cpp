...
void QImagePlaneTransformationPrivate::initTables()
{
    ...
    //qDebug() << "QImagePlaneTransformationPrivate::initTables" << srcClippedRect << dstClippedRect;
...
void QImagePlaneTransformation::setSrcGeometry( const QRect& srcRect, const QRect& srcArea, int srcLineStep )
{
    d->srcRect = srcRect.normalized();
    d->srcArea = srcArea.normalized();
    d->srcLineStep = srcLineStep;
    d->isModified = true;
}

void QImagePlaneTransformation::setDstGeometry( const QRect& dstRect, const QRect& dstArea, int dstLineStep )
{
    d->dstRect = dstRect.normalized();
    d->dstArea = dstArea.normalized();
