...
void EmbeddableKeyboardWidget::layoutBoard()
{
    ...
    int x = 0;
    int y = 0;
    int w = m_config.keySize.width();
    int h = m_config.keySize.height() - 2 * m_config.bottomMargin - 5;
    m_boardRect = QRect(x, y, w, h);
    m_boardSize = m_boardRect.size();
    
    const int spaceWidth = w/2.5;
    m_spaceRect = QRectF(x+(w - spaceWidth)/2, y+h+1, spaceWidth, m_config.bottomMargin*2);
    int sideWidth = m_spaceRect.left();
    m_periodRect = QRectF(m_spaceRect.right()+5,y+h+1,(sideWidth-17)/4,m_config.bottomMargin*2);
    m_atRect = QRectF(m_periodRect.right()+5,y+h+1,(sideWidth-17)/4,m_config.bottomMargin*2);
    m_returnRect = QRectF(m_atRect.right()+5,y+h+1,(sideWidth-17)/4 * 2+3,m_config.bottomMargin*2);
    m_modeRect = QRectF(2, y+h+1, sideWidth-7, m_config.bottomMargin*2);
    m_shiftRect = QRectF(2,h-m_config.bottomMargin*2-2,44,m_config.bottomMargin*2);
    m_backspaceRect = QRectF(w - 46,h-m_config.bottomMargin*2-2,44,m_config.bottomMargin*2);
#endif
