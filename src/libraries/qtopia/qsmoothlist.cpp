...
void QSmoothListPrivate::syncToModel()
{
    if (m_model) {
        ...
        if (m_model->rowCount()) {
            if ((selectionMode != QSmoothList::NoSelection) && (m_focusItem == -1))
                m_focusItem = 0;
...
ItemHighlight::ItemHighlight()
    : m_width(0), m_pos(0.0f), m_height(0.0f), m_listPos(0.0f), m_alpha(0.0f), m_cacheValid(false)
{
}
...
: QWidget(parent, flags), d(0)
{
    ...
    d->m_highlight = new ItemHighlight();
    ...
    d->defaultViewOptionsV3(this); //initialize view options
}
...
#ifdef BRCM1103_HARDWARE
#include "../../../devices/brcm1103/src/plugins/qtopiacore/gfxdrivers/brcm1103/brcm1103screen.h"
#endif
...
#ifdef BRCM1103_HARDWARE
    int movement = d->m_listPosition.value() - d->m_lastPaintPosition;
    d->m_lastPaintPosition = d->m_listPosition.value();
    QPoint pos = mapToGlobal(QPoint(0,0));
    if (movement != 0 && abs(movement) < height()) {
        bool up = movement > 0;
        
        QRect sourceRect(pos.x(), up ? pos.y() + movement: pos.y(), width(), height() - abs(movement));
        QPoint destinationPoint(pos.x(), up ? pos.y() : pos.y() - movement);
        static_cast<Brcm1103Screen*>(QScreen::instance())->blockCopy(sourceRect, destinationPoint);
        QRect destinationRect(destinationPoint.x()-pos.x(), destinationPoint.y()-pos.y(), sourceRect.width(), sourceRect.height());
        changeRect = changeRect.subtract(destinationRect);
        /*if (changeRect.contains(destinationRect))
            changeRect = changeRect.subtract(destinationRect);
        else
            changeRect = QRegion(rect()).subtract(changeRect);*/
    }
...
void QSmoothList::selectTimeout()
{
    ...
    if (d->m_mouseClick && selectionMode() != NoSelection) {
        ...
        if ((idx != -1) && (idx != d->m_focusItem)) {
            ...
            d->m_highlightAlpha.move(*d->m_highlight->alpha(), 1.0f, slow?rate*6:rate*3);

            correctHighlight(d->m_focusItem, d->mapToItemIndex(oldIndex)); // Fixup position correctly overrides this
        }
    }
}
        ...
        if (data && (xpos < ((data->depth() + 1) * ListItem::LevelIndent))) {
            ...
            if (selectionMode() != NoSelection) {
                ...
                if ((idx != -1) && (idx != d->m_focusItem)) {
                    ...
                    correctHighlight(idx, oldItem); // Fixup position correctly overrides this
...
/*! \internal */
void QSmoothList::go(int dir, bool allowWrap)
{
    if (!d->validModel())
        return;
    ...
    if (noPreviousFocus) {
        ...
        if (focusIndex > 0) {
            ...
            if (relocate) {
                ...
                showScrollBar(rate*12);
                d->m_pauseTimeline.clear();
                d->m_pauseTimeline.pause(d->m_pauseObject, rate*12 + 1000);
            } else {
        ...
        if (focusIndex < maxIndex) {
            ...
            if (relocate) {
                ...
                d->m_position = 0;
                d->m_listTimeline.clear();
                d->m_listTimeline.move(d->m_listPosition, d->listHeight() + visibleHeight + 20, rate*12);   //20 = magic number
    ...
    if (focusIndex != -1) {
        ...
        correctHighlight(d->m_focusItem, d->mapToItemIndex(oldIndex));

        newHighlightPos = d->itemPosition(d->m_focusItem);
        newHighlightHeight = d->itemHeight(d->m_focusItem);

        if(newHighlightPos != d->m_highlight->position()->value()) {
            d->m_highlightTimeline.clear();
            if (noPreviousFocus || incompleteListMovement) {
                d->m_highlightTimeline.set(*d->m_highlight->position(), newHighlightPos);
            } else {
                d->m_highlightTimeline.move(*d->m_highlight->position(), newHighlightPos, rate*6);
            }
        }
        ...
        if(newHighlightHeight != d->m_highlight->height()->value()) {
            ...
            if (noPreviousFocus || incompleteListMovement) {
                ...
                d->m_highlightHeight.move(*d->m_highlight->height(), newHighlightHeight, rate*6);
            }
        }

        if(newHighlightAlpha != d->m_highlight->alpha()->value()) {
            d->m_highlightAlpha.clear();
            d->m_highlightAlpha.move(*d->m_highlight->alpha(), newHighlightAlpha, rate*6);
        }
...
/*! \reimp */
void QSmoothList::resizeEvent(QResizeEvent *e)
{
    const bool heightChanged(e->oldSize().height() != e->size().height());
    const bool widthChanged(e->oldSize().width() != e->size().width());
    ...
    if (widthChanged) { 
        ...
        if (!heightChanged) {
            for (int ii = 0; ii < d->m_items.count(); ++ii)
                if (d->m_items.at(ii))
                    d->m_items.at(ii)->m_item->setWidth(width());
        }
    }
    ...
    if (rowCount > 0) {
        ...
        if (-1 != idx) {
            ...
            if (animated) {
                ...
                int listPosition(d->itemPosition(idx));

                // See if we can leave the list at the prior position
                if ((oldListPosition <= listPosition) && 
                    ((oldListPosition + height()) >= (listPosition + d->itemHeight(idx)))) {
        ...
        if (animated) {
            correctHighlight(idx, oldFocusItem); // Fixup position correctly overrides this
        }
    } else if (d->m_focusItem != -1) {
...
{
    if ((d->m_focusItem == -1) || !d->validModel() || !d->validIndex(d->m_focusItem)) {
        return QModelIndex();
    } else {
...
void QSmoothList::rowsInserted(const QModelIndex &index, int first, int last)
{
    ...
    if (redrawRequired && d->m_focusItem >= 0) {
        // Ensure the highlight is correctly sized for the selected item
        correctHighlight(d->m_focusItem, d->m_focusItem);

        int newHighlightPos = d->itemPosition(d->m_focusItem);
        int newHighlightHeight = d->itemHeight(d->m_focusItem);
    ...
    if ((!preExisting) && (selectionMode() != NoSelection) && (d->m_focusItem == -1)) {
        d->m_focusItem = 0;
        d->loadItem(d->m_focusItem, width());
        setCurrentIndex(d->m_items.at(d->m_focusItem)->index(d->m_model));
    }
...
void QSmoothList::setIconSize(const QSize &size)
{
    ...
    if(d->m_model && d->m_delegate) {
        // Re-estimate the height for unloaded items, from the first item
        QStyleOptionViewItemV3 opt = d->defaultoption;
        opt.rect = QRect(0, 0, width(), d->m_itemHeightEstimate);
        d->setItemHeightEstimate(d->m_delegate->sizeHint(opt, d->m_model->index(0,0)).height());
    } else {
        d->setItemHeightEstimate(1);
    }
...
void QSmoothList::scrollTo(int idx, ScrollHint hint)
{
    ...
    int current = d->firstVisibleIndex(height(), initialPos);
    if (current < idx) {
        ...
        int highest = d->firstVisibleIndex(height(), d->itemPosition(idx) + height());
    ...
    switch(hint) {
        ...
        case EnsureVisible: {
            if (initialPos > idxPos) {
                // Put item at top
                listpos = idxPos;
            } else if ((initialPos + height()) < (idxPos + itemHeight)) {
                // Put item at bottom
                listpos = idxPos - height() + itemHeight;
            }
            break;
        }
        ...
        case PositionAtBottom:
            listpos = (idxPos + itemHeight) - height();
            break;

        case PositionAtCenter:
            listpos = idxPos - ((height() - itemHeight) / 2);
            break;
    }
...
void QSmoothList::scrollTo(qreal top, int from)
{
    ...
    if (int(top) != from) {
        ...
        d->m_listTimeline.clear();
        if (from != current)
            d->m_listTimeline.set(d->m_listPosition, from);
        d->m_listTimeline.move(d->m_listPosition, top, rate*6);
    ...
    if (d->m_focusItem != -1) {
        ...
        if (highlightPos != int(d->m_highlight->position()->value())) {
            d->m_highlightTimeline.clear();
            d->m_highlightTimeline.set(*d->m_highlight->position(), highlightPos);
        }
        if (highlightHeight != int(d->m_highlight->height()->value())) {
            d->m_highlightHeight.clear();
            d->m_highlightHeight.set(*d->m_highlight->height(), highlightHeight);
        }
        if (d->m_highlight->alpha()->value() < 0.1f) {
            d->m_highlightAlpha.clear();
            d->m_highlight->alpha()->setValue(1.0f);
        }
    }
...
/*! \internal */
void QSmoothList::correctHighlight(int newIdx, int oldIdx)
{
    if (newIdx != -1) {
        int listTop = int(d->m_listPosition.value());
        int listBottom = int(listTop) + height();
        int itemTop = d->itemPosition(newIdx);
        int itemBottom = itemTop + d->itemHeight(newIdx);

        bool adjustList(false);
        if (itemTop < listTop) {
            listTop = itemTop;
            adjustList = true;
        } else if (itemBottom > listBottom) {
            listTop = itemBottom - height();
            adjustList = true;
        }

        if (adjustList) {
            d->m_inFlick = false;
            d->m_position = listTop;
            d->m_listTimeline.clear();
            d->m_listTimeline.move(d->m_listPosition, listTop, rate*6);
        }
    }
    if (d->m_opaqueItems) {
        // If we have opaque items they draw their own highlight.
        invalidateItem(newIdx);
...
void QSmoothList::fixupPosition()
{
    ...
    if (newPosition != static_cast<int>(d->m_listPosition.value())) {
        ...
        d->m_listTimeline.clear();
        d->m_listTimeline.move(d->m_listPosition, newPosition, rate*6);
    }
}
...
void QSmoothList::showScrollBar(int pause)
{
    ...
    d->m_scrollTime.move(*d->m_scrollbar->alpha(), Scrollbar::MaxAlpha, rate*6);
}
...
void QSmoothList::hideScrollBar()
{
    ...
    d->m_scrollOn = false;
    d->m_scrollTime.clear();
    d->m_scrollTime.move(*d->m_scrollbar->alpha(), Scrollbar::MinAlpha, rate*6);
}
