...
QContentList QPLSHandler::load(const QString& filename) const
{
    ...
    if(plsfile.contains("NumberOfEntries")) {
        int numentries=plsfile.value("NumberOfEntries").toInt();
        for(int i=1;i<=numentries;i++) {
            if(plsfile.contains("File"+QString::number(i))) {
                QContent content(plsfile.value("File"+QString::number(i)).toString(), false);
                if(plsfile.contains("Title"+QString::number(i)))
                    content.setName(plsfile.value("Title"+QString::number(i)).toString());
                result+=content;
            }
        }
    }
    ...
    if (url.isValid()) {
        ...
        if (scheme == "http")
            ...
            if(d->m_playlistHandlers.contains(checkedExtension))
                d->m_playlistHandlers[checkedExtension]->save(checkedName, d->m_urls);
        }
    }
    return false;
}
...
QVariant QMediaPlaylist::data( const QModelIndex& index, int role ) const
{
    ...
    case QMediaPlaylist::Url:
        return QUrl(QLatin1String("file://")+content.fileName());
...
QContentList QMediaPlaylist::loadBasicPlaylist(const QStringList& filenames)
{
    ...
    foreach(QString filename, filenames)
        result.append(QContent(filename, false));
    return result;
}
