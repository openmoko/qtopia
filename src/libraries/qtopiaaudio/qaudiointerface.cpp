...
{
    // Don't restart if stopped by client!!!
    if(clientState == InterfaceStateInfo::UserStop) return;
...
{
    qLog(AudioState)<<MyInfo.stream<<" S<-C "<<cmd;
    QByteArray data;
    data.append(cmd);
    data.append("\n");
...
{
    qLog(AudioState)<<"QAudioInterface::startAudio()";
...
{
    qLog(AudioState)<<"QAudioInterface::stopAudio()";
