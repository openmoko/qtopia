...
public:
	...
	submenu(sm)
    {
	StartMenu *sm = startMenu();
	QPopupMenu* popup = sm->launchMenu;
	...
	if (sm) {
		...
		if ( ind >= 0 ) {
			...
			StartMenu *sm = startMenu();
			if ( sm->launchMenu == popup ) { // sanity check
			    delete sm->launchMenu;
			    sm->launchMenu = 0;
			    // Call function in server's namespace
			    QObject dummy;
			    sm->connect(&dummy,SIGNAL(destroyed()),SLOT(createMenu()));
			}
		    }
		}
	    }
