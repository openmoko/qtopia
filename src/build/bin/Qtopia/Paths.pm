...
sub get_paths()
{
    ...
    if ( basename($0) eq "configure" || $0 =~ /configure\.exe$/i ) {
        ...
        $depotpath = undef;
        for my $dir (
            # Running src/build/bin/configure
            dirname($0)."/../../..",
            # Running bin/configure.exe
            dirname(dirname($0)) ) {
            if ( -f "$dir/src/build/bin/configure" ) {
                $depotpath = $dir;
                last;
            }
        }
        ...
        ###
        ### This code must be kept in sync with src/build/bin/getpaths.sh
        ###
        ### WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING
        ###
        ...
        LOC: for my $check ( dirname($0), $ENV{PWD}, $cwd ) {
            ...
            while ( $check ) {
                ...
                if ( -f "$check/src/build/bin/configure" ) {
