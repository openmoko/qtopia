...
void QDirectPainterVideoOutput::doRenderFrame( const QVideoFrame& frame )
{
    ...
    if ( isModified() ) {
        ...
        //qDebug() << "clip region:" << paintRegion;
        ...
        foreach( QRect r, paintRegion.rects() ) {
            ...
            //qDebug() << "src rect:" << srcRect << "src area:" << srcArea;
            //qDebug() << "dst rect:" << dstRect << "dst area:" << dstArea;
            //qDebug() << "clipped src geometry:" << transformation->clippedSrcGeometry() << " clipped dst geometry:" << transformation->clippedDstGeometry();
            //qDebug() << "effective rotation:" << int(effectiveRotation())*90;
...
#endif
        screen()->setDirty( clipRegion().boundingRect() );
        //qWarning() << "time to draw the image:" << t.elapsed();
    }
