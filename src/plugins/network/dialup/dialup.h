...
class DialupImpl : public NetworkInterface2
{
public:
    DialupImpl();
    virtual ~DialupImpl();
    ...
    virtual bool setProperties( Config& cfg, const NetworkInterfaceProperties& settings );

private:
    ulong ref;
    QString logfile;
    QString dev( Config& ) const;
    QString dialString( Config& ) const;
    bool writeSystemFiles( QWidget *parent, Config& );
};
