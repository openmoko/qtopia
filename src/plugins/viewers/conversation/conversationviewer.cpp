...
bool ConversationViewer::setMessage(const QMailMessage& msg)
{
    ...
    if (message->status() & QMailMessage::Incoming) {
        ...
        conversationalist = message->from();
    } else if (message->status() & QMailMessage::Outgoing) {
        if (!message->to().isEmpty()) {
            ...
            conversationalist = message->to().first();
        }
    }
    ...
    if (!conversationalist.isNull()) {
        ...
        QModelIndex idx = model->indexFromId(message->id());
        if (idx.isValid())
