...
DesktopAudioPlugin::DesktopAudioPlugin(QObject *parent)
    : QAudioStatePlugin(parent)
{
    m_data = new DesktopAudioPluginPrivate;
   ...
   */
    ...
    // Can "output" a phone call and record it at the same time
    ADD_STATE("Phone", "PhoneHandset", tr("Phone"), 100,
              QAudio::InputOnly | QAudio::OutputOnly,
              dummy, dummy, dummy);
...
DesktopAudioPlugin::~DesktopAudioPlugin()
{
    delete m_data;
}

QList<QAudioState *> DesktopAudioPlugin::statesProvided() const
{
    return m_data->m_states;
}

Q_EXPORT_PLUGIN2(desktopaudio_plugin, DesktopAudioPlugin)

#include "desktopaudioplugin.moc"
