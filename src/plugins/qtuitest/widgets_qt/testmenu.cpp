    ...
    : TestWidget(_q), q(qobject_cast<QMenu*>(_q)), lastAction(0)
{
    ...
    QtUiTest::connect(q,    SIGNAL(hovered(QAction*)),
...
{
    if (lastAction)
        QtUiTest::disconnect(lastAction, 0, this, 0);
    lastAction = a;
    if (lastAction) {
        QtUiTest::connect(lastAction, SIGNAL(toggled(bool)),
                            this,       SLOT(on_toggled(bool)),
                            QtUiTest::Before);
        QtUiTest::connect(lastAction, SIGNAL(triggered(bool)),
...
bool TestMenu::select(QString const &item)
{
    ...
    if (mousePreferred()) {
        ...
        if (item == topItem) {
            //FIXME: Should be waiting for signal or other confirmation that key event
            //has been received
            keyClick(QtUiTest::Key_Select);
            QtUiTest::wait(200);
        }
