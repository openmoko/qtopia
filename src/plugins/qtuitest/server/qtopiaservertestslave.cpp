...
QTestMessage QtopiaServerTestSlavePrivate::startLogRead(QTestMessage const &message)
{
    ...
    QtUiTestLogReader* reader = new QtUiTestLogReader(this);
    if (!QObject::connect(reader, SIGNAL(error(QString)),   this,   SLOT(logError(QString))))
        Q_ASSERT(0);
    if (!QObject::connect(reader, SIGNAL(log(QStringList)), this,   SLOT(log(QStringList))))
        Q_ASSERT(0);
    if (!QObject::connect(reader, SIGNAL(finished()),       reader, SLOT(deleteLater())))
        Q_ASSERT(0);
