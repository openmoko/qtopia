...
int QTestWidget::x() const
{
    ...
    QWidget *parent = parentWidget(qobject_cast<QWidget*>(_instance));
    int ret_x = 0;
    while (parent) {
        ret_x += parent->x();
        parent = parentWidget(parent);
    }
    return ret_x + qobject_cast<QWidget*>(_instance)->x();
}
...
int QTestWidget::y() const
{
    ...
    return ret_y + qobject_cast<QWidget*>(_instance)->y();
}

int QTestWidget::width() const
{
    if (!_instance) return 0;
    return qobject_cast<QWidget*>(_instance)->width();
}

int QTestWidget::height() const
{
    if (!_instance) return 0;
    return qobject_cast<QWidget*>(_instance)->height();
}
...
bool QActiveTestWidgetData::scan( QWidget *aw )
{

    if (aw == 0) return false;
    if (qobject_cast<QAbstractSpinBox*>(aw)) return false;

    bool any_appended = false;
    foreach(QObject *o, aw->children()) {
        QWidget *w = qobject_cast<QWidget*>(o);
        if (!w) continue;
        if (!QTestWidgets::widgetVisible(w)) continue;
        ...
        do {
            ...
            QLatin1String cn(w->metaObject()->className());
...
{
    QString ret = label->getSelectedText(&ok);
    if (!ok) return ret;

    QWidget *w = qobject_cast<QWidget*>(label->instance())->parentWidget();
    QGroupBox *gb = 0;
    while (w) {
...
bool QActiveTestWidget::rescan( QString &error, int timeout )
{
    ...
    // FIXME: shoould we use a QElapsedTimer?
    QTime t;
    t.start();
    QWidget *aw = 0;
    while (aw == 0) {
...
//    ret += QString("\nScan time    : %1 ms").arg(scan_time);
    ...
    foreach(QTestWidget* buddy_rec, d->unresolved_tw_buddies) {
        if (buddy_rec) {
            ...
            if (vw && !vw->inherits("BuddyFocusBox") && qobject_cast<QWidget*>(vw)->isEnabled()) {
                if (first) {
...
bool QActiveTestWidget::getCurrentTitle( QString &title, QString &error, int timeout )
{
    if (!rescan(error, timeout)) return false;
    if (active_widget) {
        title = dehyphenate( active_widget->windowTitle() );
    } else {
...
bool QActiveTestWidget::widgetAt(QPoint const &p, QTestWidget *&widget, QString &error)
{
    if (!rescan(error)) return false;
    QWidget *clickedW = QApplication::topLevelAt( p );
    if (!clickedW) {
        error = QString("ERROR: No widget found at point (%1,%2)").arg(p.x()).arg(p.y());
        return false;
    }
    ...
    do {
        finalW = clickedW;
        myP -= clickedW->pos();
        clickedW = clickedW->childAt(myP);
    } while (clickedW);

    widget = QTestWidgets::testWidget(finalW);
    if (!widget || !widget->instance()) {
        error = QString("ERROR: No test widget available for widget of type '%1' at point (%2,%3)").arg(finalW->metaObject()->className()).arg(p.x()).arg(p.y());
        return false;
    }
    return true;
}
