...
QTestMessage QTestSlavePrivate::currentTitle(QTestMessage const&)
{
    QTestMessage reply;
    QString title,error;
    if (QActiveTestWidget::instance()->getCurrentTitle(title,error)) {
        reply["currentTitle"] = title;
        return RET(reply, "OK");
    }
...
QTestMessage QTestSlavePrivate::widgetAt(QTestMessage const &message)
{
    QTestMessage reply;
    int x = message["x"].toInt();
    int y = message["y"].toInt();
    QPoint point(x,y);
    QTestWidget *tw;
    QString error;
    if (QActiveTestWidget::instance()->widgetAt(point,tw,error)) {
        reply["widgetAt"] = tw->signature();
        return RET(reply, "OK");
    }
    return RET(reply,error);
}
...
QTestMessage QTestSlavePrivate::syntaxHint(QTestMessage const &message)
{
    Q_UNUSED(message);
    QTestMessage reply;
    bool ok;
    QString sel_txt, error;

    reply["optionsMenu"] = "";
    reply["queryPath"] = """""";
    QTestWidget *tw;
    if (TextWidget* txtw = qtuitest_cast<TextWidget*>(findWidget(OptionsMenu))) {
        reply["optionsMenu"] = txtw->selectedText();
        reply["queryPath"] = "optionsMenu()";
    }

    tw = QTestWidgets::testWidget(QActiveTestWidget::instance()->focusWidget());
    if (tw) {
        reply["focusWidget"] = tw->signature();
        reply["text"] = tw->getSelectedText(&ok);
        reply["isEditable"] = tw->isEditableType();
        reply["isCheckbox"] = tw->isCheckboxType();
        bool is_combobox = false;
        if (tw->instance()) {
            is_combobox = tw->instance()->inherits("QComboBox");
            QObject *parent = tw->instance()->parent();
            while (!is_combobox && parent) {
                is_combobox = parent->inherits("QComboBox");
                parent = parent->parent();
            }
        }
        reply["isCombobox"] = is_combobox;
        reply["isChecked"] = tw->isChecked(&ok) == "YES";
        reply["tabName"] = QActiveTestWidget::instance()->currentTab();
        QString title;
        if (QActiveTestWidget::instance()->getCurrentTitle(title,error)) reply["currentTitle"] = title;
        else reply["currentTitle"] = "";
    } else {
        reply["focusWidget"] = "";
        reply["text"] = "";
        reply["isEditable"] = false;
        reply["isCheckbox"] = false;
        reply["isCombobox"] = false;
        reply["isChecked"] = false;
        reply["tabName"] = "";
        reply["currentTitle"] = "";
    }
    reply["currentLabelText"] = QActiveTestWidget::instance()->currentLabelText();
    return RET(reply, "OK");
}
...
void QTestSlave::processMessage( QTestMessage *msg )
{
    ...
    if (QAlternateStack::isAvailable()) {
        QAlternateStack* stack = new QAlternateStack(1048576, this);
...
bool QTestSlavePrivate::grabPixmap( QWidget *widget, QPixmap &pixmap, QString &error )
{
    ...
    if (!widget) {
        ...
        if (widget) {
            QWidget *w = QApplication::desktop();
            WId wid = w->winId();
            pixmap = QPixmap::grabWindow(wid, widget->x(), widget->y(), widget->width(), widget->height());
            error = "OK";
            return true;
        }
    }
