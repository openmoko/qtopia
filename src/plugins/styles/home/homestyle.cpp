    ...
    case CE_PushButtonLabel:
        if (const QStyleOptionButton *button = qstyleoption_cast<const QStyleOptionButton *>(opt)) {
            ...
            if (!button->icon.isNull()) {
                ...
                int frame = pixelMetric(PM_DefaultFrameWidth, button, widget);
    ...
    case CC_Slider: {
        if (const QStyleOptionSlider *slider = qstyleoption_cast<const QStyleOptionSlider *>(opt)) {
            ...
            if (vertical) {
                ...
                QPoint valueCenter (
                        qMin (qMax ((double)rct.x() + radius, (double)0), (double)rct.right() - radius),
                        qMin (qMax ((double)rct.y() + radius, (double)(rct.y()+normalized*rct.height())), (double)rct.bottom() - radius)
                        );
                QPainterPath path;
                ...
                QPainterPath path;
                QPoint valueCenter (
                        qMin (qMax ((double)rct.x() + (normalized*rct.width()), (double)rad), (double)rct.right() - rad),
                        rad*2 - (rad>>1)
                        );
