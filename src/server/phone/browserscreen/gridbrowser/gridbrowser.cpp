...
#ifdef QT_QWS_DYNAMIC_TRANSFORMATION
        Q_ASSERT(!rotatedPhoneLauncher);
        // swap rows and columns for rotated grid
        QSettings rcfg(Qtopia::defaultButtonsFile(),
                      QSettings::IniFormat); // No tr
        int newRows =  cfg.value("Menu/Columns", 3).toInt();
        int newColumns = cfg.value("Menu/Rows", 3).toInt();
        rcfg.setValue("Menu/Rows",newRows);
        rcfg.setValue("Menu/Columns", newColumns );

        rotatedPhoneLauncher = new PhoneMainMenu(rcfg, stack);
        QObject::connect(rotatedPhoneLauncher, SIGNAL(clicked(QContent)),
                          this, SLOT(execContent(QContent)));


        stack->addWidget(rotatedPhoneLauncher);
        stack->setCurrentWidget(phoneLauncher);
        rcfg.setValue("Menu/Rows",newColumns );
        rcfg.setValue("Menu/Columns", newRows);
#endif
...
void PhoneBrowserStack::raiseView(const QString &view, bool reset)
{
    ...
    if("Main" == view) {
        ...
        if (!gridNeedsRotation) {
            ...
            stack->setCurrentWidget(rotatedPhoneLauncher);

            if(reset)
                rotatedPhoneLauncher->showDefaultSelection();

            cur = rotatedPhoneLauncher->currentItem();
#endif
        }
...
void PhoneBrowserStack::rotationChanged()
{
    ...
    if (stack->currentWidget() == phoneLauncher
        || stack->currentWidget() == rotatedPhoneLauncher )
...
#ifdef QT_QWS_DYNAMIC_TRANSFORMATION
    else if(stack->currentWidget() == rotatedPhoneLauncher)
        rotatedPhoneLauncher->setBusy(true);
...
#ifdef QT_QWS_DYNAMIC_TRANSFORMATION
    if(stack->currentWidget() == rotatedPhoneLauncher)
        if ( rotatedPhoneLauncher ) {
            rotatedPhoneLauncher->setBusy(false);
    }
#endif
}
