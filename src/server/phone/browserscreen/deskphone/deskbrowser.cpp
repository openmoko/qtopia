...
void HomeBrowserDelegate::drawDisplay(QPainter *painter, const QStyleOptionViewItem &option, const QRect &rect, const QString &text) const
{
    int adj;
    if (bg.isNull())
        adj = (rect.width() - maxSize.width())/2;
    else
        adj = (rect.width() - bg.width())/2;
    QRect myRect = rect.adjusted(adj, -painter->fontMetrics().height(), -adj, -painter->fontMetrics().height());
    QtopiaItemDelegate::drawDisplay(painter, option, myRect, text);
}
