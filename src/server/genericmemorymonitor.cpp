...
/*!
  ...
  This might be the wrong way to do this, because it uses two
  text streams to read one file. The problem is that the file
  being read is in /proc, and the files in /proc all have 0
  size, which means the QTextStream::atEnd() function can't
  be used when reading the file directly.	

  Here we first read the entire file with QTextStream::readAll(),
  which does work, but then we use the resulting QString to
  construct a second text stream, where we can use atEnd() in
  the normal way.
  
  Note that the structure of meminfo changed in version 2.6
  of the linux kernel, so this function might need modification
  when this memory monitor is used in a linux 2.4 system.
 */
void
