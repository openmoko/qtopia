...
void *MediaPacketBuffer::startThreadLoop(void *arg)
{
	...
	while ( context->isOpen() ) {
	    ...
	    context->bufferMutex.timedWait(500000); // Buffer has been filled, wait 500ms for buffer to get used a bit
...
MediaPacket *MediaPacketBuffer::getPacket()
{
    ...
    while (!avFormatContext || av_read_packet(avFormatContext, &avPkt) < 0) {
    ...
    if ((unsigned long long)avPkt.pts != AV_NOPTS_VALUE && avFormatContext->pts_den)
	pkt->msecOffset = avPkt.pts * 1000 * avFormatContext->pts_num / avFormatContext->pts_den;
    else
        pkt->msecOffset = 0;
