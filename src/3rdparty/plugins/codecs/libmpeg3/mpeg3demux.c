...
int mpeg3_read_program(mpeg3_demuxer_t *demuxer)
{
	...
	if ( header != MPEG3_PACK_START_CODE ) {
		...
		int count = 0;
		/* Search forward */
		int mx = demuxer->packet_size ? demuxer->packet_size-1 : 256;
		while(!result && header != MPEG3_PACK_START_CODE && count < mx)
		{
		    header <<= 8;
		...
		if ( header == MPEG3_PACK_START_CODE )
		    result = mpeg3io_seek_relative(title->fs, -4);
		else
		    result = mpeg3io_seek_relative(title->fs, -count);
	    }
	    if ( demuxer->packet_size ) {
		/* Search backward */
		int count=0;
		while(header != MPEG3_PACK_START_CODE && !result && count < demuxer->packet_size-1)
		{
		...
		if ( header != MPEG3_PACK_START_CODE )
		    mpeg3io_seek_relative(title->fs, count);
	    }
	    if ( header != MPEG3_PACK_START_CODE )
		return 1;
	}
...
mpeg3demux_timecode_t* mpeg3_append_timecode(mpeg3_demuxer_t *, 
		mpeg3_title_t *title, 
		long prev_byte, 
		double prev_time, 
		long next_byte, 
...
int mpeg3demux_assign_programs(mpeg3_demuxer_t *demuxer)
{
	...
	//int previous_timecode;
	double current_time;
	//double current_length;
	//int done = 0;
	//int interleaved = 0;
	mpeg3demux_timecode_t *timecode1;
	//mpeg3demux_timecode_t *timecode2;
	...
	for(i = 0, total_timecodes = 0; i < demuxer->total_titles; i++)
		total_timecodes += titles[i]->timecode_table_size;
