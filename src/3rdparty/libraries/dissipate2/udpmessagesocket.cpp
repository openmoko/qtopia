...
UDPMessageSocket::UDPMessageSocket( void )
{
	...
	if ( ( socketfd = socket( AF_INET, SOCK_DGRAM, 0 ) ) == -1 ) {
		sip_perror( "UDPMessageSocket::UDPMessageSocket(): socket() failed" );
	}
	didcomplain = false;
}
...
int UDPMessageSocket::SetTOS( void )
{
	...
	if(setsockopt(socketfd,SOL_IP,IP_TOS,&tos,optlen) != 0){
		sip_perror("UDPMessageSocket::SetTOS");
                return -1 ;
	}
	return 0;
}
...
int UDPMessageSocket::send( const char *sendbuffer, unsigned int length )
{
	...
	if( ( numbytes = sendto( socketfd, sendbuffer, length, 0,
		...
		if( !didcomplain ) {
			sip_perror( "UDPMessageSocket::send(): sendto() failed" );
			didcomplain = true;
		}
		return -1;
	}
...
int UDPMessageSocket::receive( char *recvbuffer, unsigned int maxlength )
{
	...
	if( ( numbytes = recvfrom( socketfd, recvbuffer, maxlength, 0, \
		...
		sip_perror( "UDPMessageSocket::recieve(): recvfrom() failed" );
		return -1;
	}
	return numbytes;
}
...
unsigned int UDPMessageSocket::listen( unsigned int portnum )
{
	...
	if (bound) {
		...
		bound = false;
		if( ( socketfd = socket( AF_INET, SOCK_DGRAM, 0 ) ) == -1 ) {
			sip_perror( "UDPMessageSocket::listen: socket() failed" );
			return 0;
		}
	}
	...
	if ( count > 10 ) {
		sip_perror( "UDPMessageSocket::listen(): bind() failed" );
		return 0;
	}
	bound = true;
	...
	if ( getsockname( socketfd, &name, &namesize ) == -1 ) {
		sip_perror( "UDPMessageSocket::listen(): getsockname() failed" );
		return 0;
	}
 ...
 */
int UDPMessageSocket::listenOnEvenPortOS( void )
{
	struct sockaddr name;
	socklen_t namesize;
	int oldsocketfd = 0;
	...
	if (bound) {
		...
		bound = false;
		if( ( socketfd = socket( AF_INET, SOCK_DGRAM, 0 ) ) == -1 ) {
			sip_perror( "UDPMessageSocket::listenOnEvenPortOS: socket() failed" );
			return -1;
		}
	}
...
getaddress:
	...
	ourport = ntohs( ((struct sockaddr_in *) &name)->sin_port );
	sip_printf( "UDPMessageSocket: Listening on %d\n", ourport );
	if( ( portnum == 0 ) && ( ourport % 2 ) != 0 ) {
		sip_printf( "UDPMessageSocket: Retrying...\n" );
...
int UDPMessageSocket::listenOnEvenPort(int min, int max)
{
	...
	if (bound) {
		...
		bound = false;
		if( ( socketfd = socket( AF_INET, SOCK_DGRAM, 0 ) ) == -1 ) {
			sip_perror( "UDPMessageSocket::listenOnEvenPor: socket() failed" );
			return -1;
		}
	}
...
error:
	sip_perror("UDPMessageSocket::listen(): Can't find a free port in specified range");
	return -1;
...
getaddress:
	...
	if ( setsockopt( socketfd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof( one ) ) == -1 ) {
		sip_perror( "UDPMessageSocket::setsockopt SO_REUSEADDR" );
	}
	...
	ourport = ntohs( ((struct sockaddr_in *) &name)->sin_port );
	sip_printf( "UDPMessageSocket (video): Listening on %d\n", ourport );
