...
#	skip_host_tools
	...
	if [ $CLEAN = "1" ]; then      
		...
		if $QTOPIA_DEPOT_PATH/configure -verbose -device $DEVICE -prefix /opt/Qtopia/SDK/$QPEVER/x86/image -sdk /opt/Qtopia/SDK/$QPEVER/x86 -image /opt/Qtopia/SDK/$QPEVER/x86/image -debug -confirm-license -extra-qt-config="-release" -extra-qt-config="-qt-libpng" $EXTRA_OPTIONS >${OUTPUT} 2>&1; then
			echo "configure DONE"
		else
...
make_device_sdk()
{
	...
	if [ "$CLEAN" = "1" ]; then
		...
		if $QTOPIA_DEPOT_PATH/configure -verbose -device $DEVICE -platform linux-g++ /opt/Qtopia/SDK/$QPEVER/$DEVICE -sdk -image /opt/Qtopia/SDK/$QPEVER/$DEVICE/image -confirm-license $SKIP $EXTRA_OPTIONS >${OUTPUT} 2>&1; then
			echo "configure DONE"
		else
