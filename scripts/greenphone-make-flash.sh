...
optimize_prelink_all()
{
    ...
    CHROOT=$( ssh root@$GREENPHONE_IP "mktemp -d /tmp/prelink.XXXXXX" )

    ssh root@$GREENPHONE_IP "test -d $CHROOT && mount -t nfs -o nolock,tcp hostpc:$EXPORTED_ROOTFS $CHROOT"
    ssh root@$GREENPHONE_IP "test -d $CHROOT/opt/Qtopia && mount -t nfs -o nolock,tcp hostpc:$EXPORTED_QTOPIA $CHROOT/opt/Qtopia"

    ssh root@$GREENPHONE_IP "/usr/sbin/chroot $CHROOT /usr/sbin/prelink -v -a --ld-library-path=/opt/Qtopia/lib"

    ssh root@$GREENPHONE_IP "umount $CHROOT/opt/Qtopia"
    ssh root@$GREENPHONE_IP "umount $CHROOT"

    ssh root@$GREENPHONE_IP "rmdir $CHROOT"
}
...
EOF
    ...
    ssh root@$GREENPHONE_IP "test -d $CHROOT && mount -o bind,ro / $CHROOT"
    ssh root@$GREENPHONE_IP "test -d $CHROOT/opt/Qtopia && mount -t nfs -o nolock,tcp hostpc:$EXPORTED_QTOPIA $CHROOT/opt/Qtopia"

    ssh root@$GREENPHONE_IP "/usr/sbin/chroot $CHROOT /opt/Qtopia/prelink_helper.sh"
...
optimize_lids_all()
{
    ...
    CHROOT=$( ssh root@$GREENPHONE_IP "mktemp -d /tmp/lids.XXXXXX" )
    ...
    ssh root@$GREENPHONE_IP "test -d $CHROOT/proc && mount -t tmpfs none $CHROOT/proc"
    ssh root@$GREENPHONE_IP "
    ...
    ssh root@$GREENPHONE_IP "test -d $CHROOT/dev && mount -t tmpfs none $CHROOT/dev"
    ssh root@$GREENPHONE_IP "tar -C $CHROOT/dev -xzf $CHROOT/dev.tar.gz"
    ssh root@$GREENPHONE_IP "touch $CHROOT/dev/log"
    ssh root@$GREENPHONE_IP "test -d $CHROOT/var && mount -t tmpfs none $CHROOT/var"
    ssh root@$GREENPHONE_IP "tar -C $CHROOT/var -xzf $CHROOT/var.tar.gz"
    ssh root@$GREENPHONE_IP "touch $CHROOT/var/log/wtmp"
    ssh root@$GREENPHONE_IP "test -d $CHROOT/home && mount -t tmpfs none $CHROOT/home"

    ssh root@$GREENPHONE_IP "mkdir $CHROOT/etc/lids/.original; cp -a $CHROOT/etc/lids/* $CHROOT/etc/lids/.original"
    ssh root@$GREENPHONE_IP "/usr/sbin/chroot $CHROOT /etc/rc.d/sxe_boot"
    ssh root@$GREENPHONE_IP "mkdir $CHROOT/etc/lids/.pregenerated; mv $CHROOT/etc/lids/* $CHROOT/etc/lids/.pregenerated"
    ssh root@$GREENPHONE_IP "mv $CHROOT/etc/lids/.original/* $CHROOT/etc/lids; rmdir $CHROOT/etc/lids/.original"

    ssh root@$GREENPHONE_IP "umount $CHROOT/home"
    ssh root@$GREENPHONE_IP "umount $CHROOT/var"
    ssh root@$GREENPHONE_IP "umount $CHROOT/dev"

    ssh root@$GREENPHONE_IP "umount $CHROOT/proc"
...
usage()
{
    ...
    echo -e "If no options are specified defaults to --qtopia --qtopia-image $QTOPIA_BUILD_PATH\n" \
            ...
            "                         Greenphone. The IP address of the Greenphone can be\n" \
            "                         specified with the GREENPHONE_IP environment variable.\n" \
...
if [ $DEFAULT_IMAGES -eq 1 ] && [ $OPTION_FLASH_IMAGE -eq 1 ]; then
    ...
    if [ -f /opt/Qtopia/extras/images/greenphone-rootfs.tar.gz ]; then
        ...
        # Trolltech internal
        wget -q -N http://qtopiaweb.trolltech.com.au/dist/qtopia/$QPEVER/greenphone-rootfs.tar.gz
    ...
    if [ -f /opt/Qtopia/extras/images/greenphone-kernel ]; then
        ...
        # Trolltech internal
        wget -q -N http://qtopiaweb.trolltech.com.au/dist/qtopia/$QPEVER/greenphone-kernel
...
# Optimization of images requires connection to Greenphone.  Use GREENPHONE_IP
# environment variable as the IP address of the Greenphone to connect to,
...
if [ $OPTION_OPTIMIZE -eq 1 ]; then
    ...
    if ssh root@$GREENPHONE_IP "/bin/true" 2>/dev/null; then
        optimize
    else
        echo "Not optimizing, cannot connect to Greenphone at $GREENPHONE_IP"
    fi
fi
