...
#${
	...
	if ( !(Project("DEFINES") =~ /QT_NODLL/) &&
	     ((Project("DEFINES") =~ /QT_DLL/) || !$ENV{"QT_NODLL"}) ) {
	    if ( (Project("DEFINES") =~ /QT_(?:MAKE)?DLL/) ||
		 Config("qt_dll") || $ENV{"QT_DLL"} ||
		 FindHighestLibVersion($ENV{"QTDIR"} . "/lib", "qt") ) {
    ...
    if ( Config("windows") ) {
	if ( Config("console") ) {
	    Project('TMAKE_CFLAGS += $$TMAKE_CFLAGS_CONSOLE');
	    Project('TMAKE_CXXFLAGS += $$TMAKE_CXXFLAGS_CONSOLE');
