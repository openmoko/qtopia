    ...
    QMainWindow(parent, name, fl)
{
    ...
    reset = new QToolButton(QIcon(":icon/i18n/back"), tr("Back"), "", this, SLOT(resetTurn()), toolbar);
    done = new QToolButton(QIcon(":icon/done"), tr("Done"), "", this, SLOT(endTurn()), toolbar);
    scoreinfo = new ScoreInfo(toolbar);
    scoreinfo->setFont(QFont("Helvetica",10));
    new QToolButton(QIcon(":icon/finish"), tr("Close"), "", this, SLOT(endGame()), toolbar);
...
{
    QSettings cfg("Trolltech","WordGame");
    cfg.beginGroup("Game");
    cfg.setValue("DefaultName",defaultname);
    cfg.setValue("NameList", namelist.join(QString(';')));
    cfg.setValue("CurrentPlayer",gameover ? 0 : player+1);
    if ( !gameover ) {
        cfg.setValue("Rules",rules);
        if (bag) {
            bag->writeConfig(cfg);
        }
        if (board) {
    ...
    for (int p=0; p<nplayers; p++) {
        cfg.endGroup();
        cfg.beginGroup("Player"+QString::number(p+1));
        if ( gameover ) {
            cfg.remove("");
        } else {
            if (rack(p)) {
...
void WordGame::readConfig()
{
    QSettings cfg("Trolltech","WordGame");
    cfg.beginGroup("Game");
    defaultname = cfg.value("DefaultName",tr("Player")).toString();
    int currentplayer = cfg.value("CurrentPlayer",0).toInt();
    QStringList pnames = cfg.value("NameList").toString().split(';');
    if ( currentplayer ) {
        gameover = false;
        rules = cfg.value("Rules").toString();
...
void WordGame::openGameSelector(const QStringList& /* initnames */)
{
    ...
    newgame->player0->changeItem(defaultname,0);
...
void WordGame::startGame()
{
    ...
    if ( loadRules(rules) ) {
        QStringList names;
        names.append(defaultname=newgame->player0->currentText());
...
{
    QString filename = Qtopia::applicationFileName( "wordgame", name );
    QFile file( filename );
    ...
    QImage shim = QImage(":image/"+shapepixmap);
...
void NewGame::updateRuleSets()
{
    ...
    QString rulesDir = Qtopia::applicationFileName( "wordgame", "" );
...
bool WordGame::mayEndGame()
{
    ...
    if ( out<0 ) {
        if ( QMessageBox::warning(this,tr("End game"),
                                  tr("<qt>Do you want to end the game early?</qt>"),
                                  tr("Yes"), tr("No") )!=0 )
        {
            return false;
        }
...
{
    const QDawg::Node* root = dict ? Qtopia::dawg("WordGame").root()
                                : Qtopia::fixedDawg().root();
...
void Board::writeConfig(QSettings& cfg)
{
    ...
    cfg.setValue("Board", t.join(QString(';')));
}

void Board::readConfig(QSettings& cfg)
{
    clear();
    QStringList t = cfg.value("Board").toString().split(';');
    int i=0;
...
retry:
    ...
    for (QStringList::Iterator it=words.begin(); it!=words.end(); ++it) {
        if ( (*it).length() > 1 && !Qtopia::fixedDawg().contains(*it)
                && !Qtopia::dawg("WordGame").contains(*it) ) {
    ...
    if ( to_add.count() )
        Qtopia::addWords("WordGame",to_add);
    return true;
}
...
int Board::score(QPoint at, const Tile** tiles, int n, const Tile* blankvalue, const QPoint& d, bool checkdict, QStringList* words) const
{
    ...
    for (int i=0; contains(at) && ((t=tile(at)) || i<n); ) {
        if ( t ) {
            ...
            if ( sideword.length() > 1 ) {
                ...
                if ( checkdict && !Qtopia::fixedDawg().contains(sideword)
                               && !Qtopia::dawg("WordGame").contains(sideword) )
                    return -1;
    ...
    if ( checkdict && !Qtopia::fixedDawg().contains(mainword)
                   && !Qtopia::dawg("WordGame").contains(mainword) )
        return -1;
...
void Rack::writeConfig(QSettings& cfg)
{
    QStringList l;
    for (int i=0; i<n; i++)
        l.append(tile(i).key());
    cfg.setValue("Tiles", l.join(QString(';')));
}

void Rack::readConfig(QSettings& cfg)
{
    clear();
    int x=0;
    QStringList l = cfg.value("Tiles").toString().split(';');
...
void Rack::contentsMouseReleaseEvent(QMouseEvent*)
{
    if ( computerized() )
        return;
    if ( dragging ) {
...
void Bag::writeConfig(QSettings& cfg)
{
    QStringList t;
    for (QListIterator<Tile> it(tiles); it; ++it)
        t.append((*it)->key());
    cfg.setValue("Tiles", t.join(QString(';')));
}

void Bag::readConfig(QSettings& cfg)
{
    tiles.clear();
    QStringList t = cfg.value("Tiles").toString().split(';');
    for (QStringList::ConstIterator it=t.begin(); it!=t.end(); ++it )
        add(Tile(*it));
}
...
void ScoreInfo::writeConfig(QSettings& cfg)
{
    QStringList l;
    for (int i=0; i<(int)names.count(); i++)
        l.append(QString::number(score[i]));
    cfg.setValue("Scores", l.join(QString(';')));
}

void ScoreInfo::readConfig(QSettings& cfg)
{
    QStringList l = cfg.value("Scores").toString().split(';');
    int i=0;
    for (QStringList::ConstIterator it=l.begin(); it!=l.end(); ++it )
        score[i++]=(*it).toInt();
    showScores();
