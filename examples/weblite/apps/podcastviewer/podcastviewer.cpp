    ...
    public slots:
        ...
        bool testBuffer (bool active)
        {
            if (mediaGetter)
            {
                ...
                qDebug() << QString("Download: %1/%2 bytes (%3), %4 millis. DBPMS=%5. Played %6/%7 millis, bpms=%8. available to play: %9 millis. time left to download: %10. time left to play: %11").arg(loadedBytes).arg(totalBytes).arg(totalBytes?(((double)loadedBytes/totalBytes)*100):0).arg(downloadTime).arg(dbpms).arg(playedTime).arg(duration).arg(bpms).arg(availPlayable).arg(timeLeftToDownload).arg(timeLeftToPlay);
...
int main (int argc, char** argv)
{
    ...
    if (!QDir("/mfs/sd").exists())
    {
        QMessageBox::warning(a,"No SD Card found","The Podcast Viewer cannot operate without an SD card. Please choose a different application, or install an SD card.");
        QTimer::singleShot(0,&app,SLOT(exit()));
        app.exit ();
    }
#endif
    app.exec();
