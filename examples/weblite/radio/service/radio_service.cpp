...
{
//    qDebug() << "Radio: play file" << d->mediaFilename;
    if (d->ctx)
        delete d->ctx;
    if (d->notifier)
        delete d->notifier;
...
//            qDebug() << "Radio: Received line" << line;
...
{
//    qDebug() << "WebRadioService::transition" << mainState << ms << cleanupState << cs;
...
{
    d->timer->stop ();
//    qDebug() << "WebRadioService::processEvent" << e;
    switch (e)
    {
