...
{
#ifdef QT_QWS_BRCM1103
    // on the brcm1103 platform, audio and video files should be saved on the SD card for better VC02 support
    if ((mt.startsWith ("audio/") || mt.startsWith("video/")
         //|| mt.startsWith("image/")
        )
         && QDir("/mfs/sd").exists())
    {
        return "/mfs/sd/weblite-cache";
    }
    else
#else
    Q_UNUSED(mt);
...
void WebLiteServerPrivate::loadCache ()
{
    ...
    //            qDebug() << rsp.filename << rsp.totalBytes << cacheQuota[rsp.cachePath].usedSpace;
                if (verifyFile(rsp))
                {
                    cache[rsp.url] = rsp;
...
void WebLiteServerPrivate::freeCacheSpace(const QString & cachePath, int neededSpace, const QUrl & u)
{
    ...
    if (cq->totalSpace- cq->usedSpace >= neededSpace)
        return;
//    qDebug() << "Cache needs space: " << neededSpace << cachePath << "Current: " << cq->usedSpace << "Total: " << cq->totalSpace;
    ...
    for (QMap<QUrl,WebLiteLoadResponse>::iterator it = cache.begin (); it != cache.end(); ++it)
    {
        ...
        {
//            qDebug() << "Can't erase " << curHandler->req.url << "; active";
            continue;
        }
...
//        qDebug() << "Erasing " << rsp->filename << "which has cache value of " << rsp->cacheValue << "Used space now at " << cq->usedSpace;
        QFile::remove (rsp->filename);
        cache.erase(*it2);
    }
}
...
//    qDebug() << "Aborting " << u << " now remaining clients: " << activeClients.size();
    if (activeClients.empty())
    {
//        qDebug() << "No more clients for " << req.url;
        deleteLater ();
    }
}
...
//    qDebug() << "Retreiving " << url.host() << path;
...
//                qDebug() << "Cache File " << (cacheFileOk?"OK":"invalid") << "Last modified date " << (lmvalid?"OK":"invalid") << (lastMod <= rsp.lastModified) << hdr.contentLength() << rsp.totalBytes << hdr.contentType();
                if (
...
void WebLiteServer::request    (const WebLiteLoadRequest & req)
{
    ...
    else if (req.url.scheme() == "http")
    {
        ...
        else
        {
            ...
            {
                qDebug() << "Already downloading, adding client" << h->rsp.status << h->rsp.url << h->rsp.filename << h->rsp.error;
                sendResponse(h->rsp);
            }
...
void WebLiteServer::sendResponse( WebLiteLoadResponse r)
{
    ...
    ds << r;
    if (r.status != WebLiteLoadResponse::SomeData)
        qDebug() << r.url << r.filename << r.totalBytes << r.loadedBytes << r.contentType << (int)r.status;
    if (r.status == WebLiteLoadResponse::Complete && r.url.scheme() == "http")
    {
        r.cacheValue = ++d->maxCacheValue;
        qDebug() << r.url << "has cache value of " << r.cacheValue;
        d->cache[r.url] = r;
        d->saveCache();
    }
    emit response (a);
