...
public:
    URLDialog( QWidget * parent = 0, Qt::WindowFlags flags = 0) : QDialog(parent, flags)
    {
        ...
        QSoftMenuBar::setLabel(urlEdit, Qt::Key_Back, "go", tr("Go"));  //TODO: image
    }
...
void QtopiaPluginFactory::refreshPlugins()
{
    ...
    qDebug() << "we have" << mPlugins.count() << "plugins";
}
...
QObject* QtopiaPluginFactory::create(const QString& mimetype, const QUrl& url, const QStringList& args, const QStringList& values) const
{
    ...
    Q_UNUSED(values);
    QObject *ret = 0;
    qDebug() << "Creating plugin for:" << mimetype;
...
static const int vcursorjump=15;
...
void QtopiaWebView::keyPressEvent(QKeyEvent* e)
{
    if (!Qtopia::mousePreferred()) {
        ...
        case Qt::Key_Select:
            {
                ...
                QApplication::postEvent(this,new QMouseEvent(QEvent::MouseButtonRelease,vcursorpos,Qt::LeftButton,Qt::LeftButton,0));
                return;
            }
            break;
        case Qt::Key_Left:
        ...
        if (!movecursor.isNull()) {
            vcursorfade.start(1000,this);
            if (!rect().contains(vcursorpos+vcursorjump*movecursor)) {
                // at edge - just scroll
                ...
                for (int i=0; i<vcursorjump; ++i) {
                            ...
                            tohit.imageUrl() != fromhit.imageUrl()))
                    {
                        ...
                        if (!newhit.linkUrl().isEmpty() || !newhit.imageUrl().isEmpty())
                            i -= vcursorjump; // look further (so we can center on new hit better)
                    } else if (havenewhit) {
        ...
        downloading(false)
{
    ...
    QAction *actionSpeedDial = new QAction(QIcon("speeddial"),tr("Add to Speed Dial..."),this);
    ...
    QString homestr = conf.value("Home").toString();
    homestr.replace("$QPEDIR",Qtopia::qtopiaDir());
    QUrl home(homestr);
...
void WebViewer::handleUnsupportedContent(QNetworkReply* reply)
{
    ...
    if (handler.isNull()) {
        // XXX warn? save?
        qDebug() << "WebViewer::handleUnsupportedContent"
            << reply->url()
    ...
    if (QtopiaService::apps("PlayMedia").contains(handler.executableName())) {
        if (mimetype.id().startsWith("audio")) { // XXX Helix doesn't do http video properly
            ...
            qDebug() << "Stream" << reply->url().toString() << "via" << handler.name();
    ...
    qDebug() << "download to" << downloadcontent.file();
...
{
    qDebug() << "WebViewer::finishedDownload";
    downloadto.close();
...
void WebViewer::addToSpeedDial()
{
    ...
    QString sd = QSpeedDial::addWithDialog(label,iconData,req,this);
}
