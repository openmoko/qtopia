...
# define MIN_CELL_SIZE  15 // Choose a smaller arbitrary number for smaller screen
#else
...
{
    QTablePrivate() {
	updateSurroundingCells = FALSE;
    }
    bool updateSurroundingCells;
};
			...
			const QRect &cr, bool selected )
{
	...
	if ( !table()->isModalEditing() ) {
	    if (table()->viewport()->backgroundMode() == QWidget::PaletteBackground)
...
void QTable::drawContents( QPainter *p, int cx, int cy, int cw, int ch )
{
    ...
    int collast = ( (columnPos( colfirst ) + columnWidth( colfirst )) >= (cx + cw) ) ? colfirst : columnAt( cx + cw );
    int rowfirst = rowAt( cy );
    int rowlast = ( (rowPos( rowfirst ) + rowHeight( rowfirst )) >= (cy + ch) ) ? rowfirst : rowAt( cy + ch );
			...
			const QRect &cr, bool selected )
{
	...
	if( qt_modalEditingEnabled ) {
	    if ( !isModalEditing() ) {
		if (viewport()->backgroundMode() == PaletteBackground)
		    empty = QColorGroup::Background;
	    }
	}
#endif
	p->fillRect( 0, 0, w, h, selected ? colorGroup().brush( QColorGroup::Highlight ) : colorGroup().brush( empty ) );
    }
...
void QTable::setItem( int row, int col, QTableItem *item )
{
    ...
    // this is already called in clearCell()
...
void QTable::updateCell( int row, int col )
{
    ...
    int m = 0;
    if( d->updateSurroundingCells ) // cell is/was being edited; update surrounding cells also
	m = 2;
    QRect r( contentsToViewport( QPoint( cg.x() - m, cg.y() - m ) ),
	     QSize( cg.width() + m*2, cg.height() + m*2 ) );
...
void QTable::setNumRows( int r )
{
	...
	leftHeader->QTableHeader::setNumLabels( r );
	leftHeader->QHeader::setNumLabels( r );
    } else {
	clearSelection( FALSE );
	while ( numRows() > r )
...
QWidget *QTable::beginEdit( int row, int col, bool replace )
{
    ...
    e->setFocus();
    d->updateSurroundingCells = TRUE;
    updateCell( row, col );
    d->updateSurroundingCells = FALSE;
    return e;
}
...
void QTableHeader::setNumLabels( int n )
{
    int olds = states.size();
    states.resize( n );
    for (int i = olds; i < n; ++i) {
	states[i] = Normal;
    }
}
...
void QTableHeader::updateStretches()
{
	...
	for ( i = 0; i < (int)stretchable.count(); ++i ) {
	    ...
	    resizeSection( i, QMAX( MIN_CELL_SIZE, pw ) );
	}
	blockSignals( block );
	...
	for ( i = 0; i < (int)stretchable.count(); ++i ) {
	    ...
	    resizeSection( i, QMAX( MIN_CELL_SIZE, ph ) );
	}
	blockSignals( block );
	table->viewport()->repaint( FALSE );
	widgetStretchTimer->start( 100, TRUE );
