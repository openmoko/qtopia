/****************************************************************************
** $Id: qt/src/widgets/qwidgetstack.cpp   2.3.12   edited 2005-10-27 $
**
...
void QWidgetStack::raiseWidget( QWidget * w )
{
	...
	if ( w->focusPolicy() != QWidget::NoFocus ) {
	    ...
	    QWidget * fc = 0;
	    bool done = FALSE;
	    const QObjectList * c = w->queryList("QWidget");
	    if ( c ) {
		QObjectListIt it( *c );
		QObject * wc;
		while( !done && (wc=it.current()) != 0 ) {
			...
			if ( f == wc ) {
				    ...
				    == QWidget::TabFocus && !((QWidget *)wc)->isTopLevel()) {
...
void QWidgetStack::show()
{
	...
	while( (o=it.current()) != 0 ) {
	    ++it;
	    if ( o->isWidgetType() && !((QWidget*)o)->isTopLevel() )
		if ( o == topWidget )
		    ((QWidget *)o)->show();
