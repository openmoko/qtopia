/**********************************************************************
** $Id: qt/src/widgets/qcombobox.cpp   2.3.12   edited 2005-10-27 $
**
...
QSize QComboBox::sizeHint() const
{
    ...
    int sw, sh;
    if ( d->usingListBox() ) {
	int fm = style().pixelMetric(QStyle::ComboBoxFocusMargin); // 1
	int hm = style().pixelMetric(QStyle::ComboBoxTextHMargin); // 2
	int vm = style().pixelMetric(QStyle::ComboBoxTextVMargin); // 1
	int xvm = style().pixelMetric(QStyle::ComboBoxTextVMarginExtra); // 1
	sw = (fm+hm+1)*2 + maxW;
	sh = (fm+vm+xvm)*2 + 4 + maxH;
    ...
    if ( sh <= il+5 && style() == WindowsStyle || parentWidget() &&
...
void QComboBox::paintEvent( QPaintEvent * )
{
	...
	QRect textR(tmpR.x()+fm, tmpR.y()+fm, tmpR.width()-fm*2, tmpR.height()-fm*2);
	...
	textR.setRect(tmpR.x()+hm, tmpR.y()+vm, tmpR.width()-hm*2, tmpR.height()-vm*2);
	p.setClipRect( textR );
	...
	if ( !d->ed ) {
	    ...
	    const QPixmap *pix = item->pixmap();
	    if ( pix ) {
		p.fillRect( r.x(), r.y(), pix->width() + hm*2, r.height(), colorGroup().brush( QColorGroup::Base ) );
		p.drawPixmap( r.x() + hm, r.y() + ( r.height() - pix->height() ) / 2, *pix );
	    }
	}
	p.setClipping( FALSE );
    }
...
void QComboBox::keyPressEvent( QKeyEvent *e )
{
	...
	switch( e->key() ) {
	    case Key_Select:
		if ( !(d->ed && d->ed->isModalEditing()) && !isModalEditing() ) {
		    setActive = TRUE;
		    if ( d->ed )
			d->orgTxt = d->ed->text();
		} else {
		    if ( d->ed ){
			d->ed->setModalEditing( FALSE );
			update();
		    }else{
	    ...
	    case Key_No:
		if ( isModalEditing() ) {
		    if (  d->ed && d->ed->text() != d->orgTxt )
			d->ed->setText( d->orgTxt );
		    setModalEditing( FALSE );
		} else {
		    e->ignore();
		}
		return;
	    default:
		if ( !isModalEditing() || (d->ed && !d->ed->isModalEditing()) ) {
		    e->ignore();
		    return;
		}
	}
    ...
    if ( ( e->key() == Key_F4 && e->state() == 0 ) ||
	 ...
	 ( !d->ed && e->key() == Key_Space  || setActive ) ) {
	if ( count() ) {
	    if ( !d->usingListBox() )
...
void QComboBox::popup()
{
	...
	if (y + h > sh ) {
	    if ( sh - y > y ) {
		d->listBox()->resize(w, sh - y);
	    } else {
		if ( y - h - height() >= 0 ) {
		    y = y - h - height();
		} else {
		    d->listBox()->resize(w, y);
		    y = 0;
		}
	    }
	}
...
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled )
	d->listBox()->setModalEditing( TRUE );
#endif
}
...
void QComboBox::popDownListBox()
{
    ...
    if( qt_modalEditingEnabled ) {
	if ( d->ed )
	    d->ed->setModalEditing( TRUE );
	else
	    d->listBox()->setModalEditing( FALSE );
    }
#endif
...
bool QComboBox::eventFilter( QObject *object, QEvent *event )
{
	...
	if ( event->type() == QEvent::KeyPress ) {
#ifdef QT_KEYPAD_MODE
	    if( qt_modalEditingEnabled ) {
	    // can't pass these back to QWidget::event as they will be interpreted as TAB
	    ...
	    update();
#ifdef QT_KEYPAD_MODE
	} else if ( event->type() == QEvent::LeaveModalEdit || event->type() == QEvent::EnterModalEdit ) {
	    if( qt_modalEditingEnabled ) {
		update();
	    }
#endif
