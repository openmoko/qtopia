/****************************************************************************
** $Id: qt/src/widgets/qtextbrowser.cpp   2.3.12   edited 2005-10-27 $
**
...
public:
    ...
    QString curhref;
    QtTriple curhrefstartpos;
    QtTriple curhrefendpos;
    QtTriple prevhrefstartpos;
#endif
};
...
{
#ifdef QT_KEYPAD_MODE
    bool hadCurrent = d->curhref.length() > 0;
    d->curhref = "";
#endif
    QTextView::setText( text, context );
#ifdef QT_KEYPAD_MODE
    // select anchor if it's visible
    if( !selectNextPrevHref(TRUE) && hadCurrent ) 
	// no anchor to scroll to and had one previously, so notify
...
void QTextBrowser::keyPressEvent( QKeyEvent * e )
{
	...
	case Key_Select:
		if ( qt_modalEditingEnabled && !isModalEditing() ) {
		    ...
		    if ( containsAnchor() )
			richText().setSelectedAnchor( d->curhref, 
						    d->curhrefstartpos, d->curhrefendpos );
		    return;
		} else if ( !d->curhref.isEmpty() ) {
		    setSource( d->curhref );
		    return;
		} else {
		    richText().setSelectedAnchor( QString::null, 
						QtTriple(), QtTriple() );
		    if( qt_modalEditingEnabled ) setModalEditing( FALSE );
		}
	    break;
	case Key_Back:
	case Key_No:
	    if ( qt_modalEditingEnabled && isModalEditing() ) {
		if ( !d->curhref.isEmpty() )
		    richText().setSelectedAnchor( QString::null, 
					    QtTriple(), QtTriple() );
	...
	case Key_Down:
		...
		sb->setValue( sb->value() + (sb->pageStep() >> 1) );
...
/*!
    \Internal
*/
bool QTextBrowser::selectNextPrevHref(bool next)
{
    QRichTextIterator it( richText() );
    bool foundCurrent = d->curhref.isEmpty();
    QString found;
    QtTriple startPos, endPos;
    QtTriple prevprev;
    QString anchor; //, atAnchor;
    QtTriple anchorPos;
    bool anchorBroken = FALSE;
    do {
	anchor = it.format()->anchorHref();
	anchorPos = it.position();
	if ( !anchor.isEmpty() ) { // it's an anchor
	    if ( !next ) {
		//select a previous link
		if (it.position() == d->prevhrefstartpos)  {
		    //at the link before the current
		    // break if previous anchor isn't within the current viewport
		    if ( it.lineGeometry().top() < viewport()->rect().top() + contentsY() )
			break;
		    found  = anchor;
		    startPos = anchorPos;
		    endPos = startPos;
		    it.right( FALSE );
		    while( it.format()->anchorHref().length() )
		    {
			endPos = it.position();
			it.right( FALSE );
		    }
		    d->prevhrefstartpos = prevprev;
		    break;
		} else //if ( anchor != atAnchor ) 
		{
		    //keep track of current link until we get to new current. 
		    // prevprev becomes prev href
		    if( prevprev.isNull() || anchorBroken )
		    {
			//haven't set yet or this is a new link
			prevprev = anchorPos;
			anchorBroken = FALSE;
		    }
		}
		...
		if ( d->curhref.isEmpty() || 
				(foundCurrent && d->curhrefendpos < anchorPos)  ) {
		    //there's no current or we've found the current and this is 
		    //the anchor after it
		    // break if next anchor isn't within the current viewport
		    if ( it.lineGeometry().bottom() >= viewport()->rect().bottom() + contentsY() )
			break;
		    d->prevhrefstartpos = d->curhrefstartpos;
		    found = anchor;
		    startPos = anchorPos;
		    endPos = startPos;
		    it.right( FALSE );
		    ...
		    break;
		} else if ( !foundCurrent && d->curhrefstartpos == anchorPos ) {
		    foundCurrent = TRUE;
		    //don't set prevpos until we know there's a link after it
		}
	    }
	}
	else
	{
	    //rich text other than an anchor
	    anchorBroken = TRUE;
	}
    } while ( it.right( FALSE ) );
    ...
    if ( found.length() ) {
	d->curhref = found;
	d->curhrefstartpos = startPos;
	d->curhrefendpos = endPos;
	richText().setSelectedAnchor( d->curhref, 
						d->curhrefstartpos, d->curhrefendpos );
	...
	// erase previous highlighted link
	if ( next ) {
	    updateContents(0, 0, contentsWidth(), contentsHeight());
	} else {
	    QString tmp = d->curhref;
	    do { 
		if ( it.format()->anchorHref().length() && tmp != it.format()->anchorHref() )
		    break;
	    } while ( it.right( FALSE ) );

	    //updateContents( it.lineGeometry() );
	    updateContents(0, 0, contentsWidth(), contentsHeight());
	}
	
	emit highlighted( d->curhref );
	return TRUE;
    }
    return FALSE;
}
...
/*
    \Internal
*/
bool QTextBrowser::containsAnchor() const
{
    QRichTextIterator it( richText() );
    do {
	if ( !it.format()->anchorHref().isEmpty() )
	    return TRUE;
    } while ( it.right( FALSE ) );
    return FALSE;
}
