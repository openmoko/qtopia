/****************************************************************************
** $Id: qt/src/widgets/qbutton.cpp   2.3.12   edited 2005-10-27 $
**
...
void QButton::keyPressEvent( QKeyEvent *e )
{
    ...
    case Key_Left:
#ifdef QT_KEYPAD_MODE
	if( !qt_modalEditingEnabled || e->key() == Key_Up ) {
#endif
# ifndef QT_NO_BUTTONGROUP
	if ( group() )
...
# endif
	...
	if( e->key() != Key_Up && qt_modalEditingEnabled )
	    e->ignore();// won't reach default case so must ignore here
#endif
	break;
    case Key_Down:
    case Key_Right:
#ifdef QT_KEYPAD_MODE
	if( !qt_modalEditingEnabled || e->key() == Key_Down ) {
#endif
# ifndef QT_NO_BUTTONGROUP
	    if ( group() )
		group()->moveFocus( e->key() );
...
# endif
	...
	if( e->key() != Key_Down && qt_modalEditingEnabled )
	    e->ignore(); // won't reach default case so must ignore here
#endif
	break;
    case Key_Escape:
