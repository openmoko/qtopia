/****************************************************************************
** $Id: qt/src/widgets/qspinbox.cpp   2.3.12   edited 2005-10-27 $
**
...
QSize QSpinBox::sizeHint() const
{
    ...
    int wx = fm.width( ' ' )*2 * style().pixelMetric(QStyle::SpinBoxHExtraScale);
    QString s;
    ...
    QSize r( (h*style().pixelMetric(QStyle::SpinBoxButtonScale)) // buttons AND frame both sides - see resizeevent()
	     + style().pixelMetric(QStyle::SpinBoxHMargin) // right/left margins
	     + w, // widest value
	     frameWidth() * 2 // top/bottom frame
	     + style().pixelMetric(QStyle::SpinBoxVMargin) // top/bottom margins
	     + h // font height
	     );
...
bool QSpinBox::eventFilter( QObject* obj, QEvent* ev )
{
    ...
    if ( ev->type() == QEvent::FocusOut || ev->type() == QEvent::Hide ) {
	if ( edited )
	    interpretText();
#ifdef QT_KEYPAD_MODE
	if( qt_modalEditingEnabled )
	    update();
    } else if ( qt_modalEditingEnabled && ev->type() == QEvent::FocusIn ) {
	if ( ((QFocusEvent *)ev)->reason() != QFocusEvent::Mouse ){
	    vi->selectAll();
	    update();
	}
    } else if ( qt_modalEditingEnabled && 
	    (ev->type() == QEvent::EnterModalEdit || ev->type() == QEvent::LeaveModalEdit)) {
	update();
#endif
	...
	if( qt_modalEditingEnabled ) {
		...
		case Key_Back:
		case Key_No:
		    // if vi is the modal editor we know it will get the event after us,
		    // so don't eat it here as it needs to restore the original value.
		    if ( isModalEditing() )
			setModalEditing( FALSE );
		    break;
		default:
		    if ( !isModalEditing() && !vi->isModalEditing() )
			return FALSE;
	    }
	}
#endif
