/****************************************************************************
** $Id: qt/src/widgets/qscrollview.cpp   2.3.12   edited 2005-10-27 $
**
...
void QScrollView::vslide( int pos )
{
	...
	if ( testWState(WState_GlobalBrushOrigin) ) {
	    // Reduce flicker by moving line at a time.
	    if (pos < verticalScrollBar()->maxValue()) {
		int step = verticalScrollBar()->lineStep();
...
void QScrollView::updateScrollBars()
{
	...
	if (cornerWidget()) {
	    bottom = h-hsbExt;	// must reserve space for corner button
	    if ( style() == WindowsStyle )
