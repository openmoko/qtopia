/**********************************************************************
** $Id: qt/src/widgets/qlineedit.cpp   2.3.12   edited 2005-10-27 $
**
...
#if defined( _WS_QWS_ ) && !defined( QT_NO_QWS_IM )
...
#ifndef QT_NO_QWS_IM
        ,preeditStart(-1), preeditLength( 0 ), preeditCPos(-1), preeditSelLen(0)
#endif
#ifdef QT_KEYPAD_MODE
	,deleteAllTimerId(-1)
#endif
...
#ifdef QT_KEYPAD_MODE
    // In keypad mode, we can show the start (most meaningful)
    // since we go to the end as modal mode is entered.
...
#if defined (_WS_QWS_)
bool qt_lineedit_password_visible_on_focus = FALSE;
#endif
...
void QLineEdit::keyPressEvent( QKeyEvent *e )
{
	...
	default:
	    if( qt_modalEditingEnabled ) {
		if ( !isModalEditing() && !(e->state() & ControlButton) ) {
		    if ( e->text()[0].isPrint() ) {
...
#ifdef _WS_QWS_
    if ( qt_lineedit_password_visible_on_focus &&
	 echoMode() == Password &&
#ifdef QT_KEYPAD_MODE
	 e->key() != Key_Select &&
#endif
...
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !select && !isModalEditing() ) {
	setModalEditing( TRUE );
	if ( e->key() == Key_Select )
	...
	case Key_Back:
		...
		if (!e->isAutoRepeat()) {
		    if (isReadOnly()) {
			setModalEditing( FALSE );
		    } else if (tbuf.length() == 0) {
			setText(d->orgTxt);
			setModalEditing( FALSE );
		    } else if ( d->deleteAllTimerId < 0) {
...
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled ) {
	if ( isModalEditing() ) // && e->reason() != QFocusEvent::Popup )
	    setModalEditing( FALSE );
    }
#endif
...
extern QPoint qt_backgroundOffset(const QWidget *w);
...
void QLineEdit::paintEvent( QPaintEvent *e )
{
	...
	if( qt_modalEditingEnabled ) {
		...
		QPoint off;
		if (parentWidget())
		    off = qt_backgroundOffset(parentWidget()) + pos();
		p.setBrushOrigin(-(off.x()+frameW()), -(off.y()+frameW()));
	    }
	}
#endif
...
#ifndef QT_NO_QWS_IM
	if ( d->preeditLength > 0 && d->preeditSelLen > 0 ) {
	    markBegin = d->preeditStart + d->preeditCPos;
	    markEnd = markBegin + d->preeditSelLen;
	}
#endif
...
#ifndef QT_NO_QWS_IM
	//We're lazy and also fill in the about-to-be-selected part
	if ( composeMode() ) {
	    QString beforePre = display.mid( 0, d->preeditStart );
	    QString preed = display.mid( d->preeditStart, d->preeditLength );
	    int bw = fm.width( beforePre );
	    int pw = fm.width( preed );

	    if ( !qt_im_compose_background ) {
		p.setPen( g.text() );
		p.drawLine( x+bw, y+2, x+bw+pw, y+2 ); 
	    } else {
		p.fillRect( x+bw, 0, pw, height(), *qt_im_compose_background );
	    }
	}
#endif
	...
	if( qt_modalEditingEnabled ) {
	    if(!isModalEditing()) {
		style().drawFocusRect( &p, QRect(0, 0, width(), height()), 
			colorGroup(), &colorGroup().highlight(), frame());
	    }
	}
#endif
...
void QLineEdit::imStartEvent( QIMEvent *e )
{
    ...
    */
    ...
    insert(""); //####
    d->preeditStart = cursorPos;
    d->preeditLength = 0;
    e->accept();


//    qDebug( "%p QLineEdit::imStartEvent txt %s cp %d sl %d cursor %d", this,
//	      e->text().latin1(), 
//	      e->cursorPos(), e->selectionLength(), cursorPos );
    d->mousePressed = FALSE; //### stop any dragging
...
void QLineEdit::imComposeEvent( QIMEvent *e )
{
    if ( isReadOnly() )
	return;
//    qDebug( "QLineEdit::imComposeEvent txt %s cp %d sl %d", 
//    e->text().latin1(), 
//	      e->cursorPos(), e->selectionLength() );
    QString test( tbuf);

    
    test.remove( d->preeditStart, d->preeditLength );
	
    d->preeditLength = e->text().length();
    test.insert( d->preeditStart, e->text() );

    d->preeditCPos = e->cursorPos();
    d->preeditSelLen = e->selectionLength();

    cursorPos = d->preeditStart + e->cursorPos();
    tbuf = test;
    d->pmDirty = TRUE;
    d->offsetDirty = TRUE;
    blinkOn();
...
void QLineEdit::forceIMEnd()
{
	...
	d->preeditCPos = -1;
	d->preeditSelLen = 0;
	// possible the below should generate an event to result in an imEnd
	// event, but that just seems too complicated.
#ifdef _WS_QWS_
...
{
//    qDebug( "%p QLineEdit::imEndEvent txt %s cp %d sl %d", this,
//	      e->text().latin1(), 
//	      e->cursorPos(), e->selectionLength() );
    ...
    test.remove( d->preeditStart, d->preeditLength );
    test.insert( d->preeditStart, e->text() );
    int newcp = d->preeditStart+e->text().length();
    ...
    //######### textChanged() probably doesn't work!
    //######### alignment doesn't work
    validateAndSet( test, newcp , newcp, newcp ); //### not a perfect fit, uses tbuf...
    update(); // since if doesn't have focus... further on won't repaint.. strange but true.
...
bool QLineEdit::composeMode() const
{
    return d->preeditStart >= 0;
}
...
#if defined( _WS_QWS_ ) && !defined( QT_NO_QWS_IM )    
    if ( composeMode() ) {
	int clickPos = xPosToCursorPos( e->pos().x() ) - d->preeditStart;
	if ( clickPos >= 0 && clickPos <= d->preeditLength ) 
	    QPaintDevice::qwsDisplay()->sendIMMouseEvent( clickPos, TRUE ); 
...
void QLineEdit::mouseReleaseEvent( QMouseEvent * e )
{
	...
	int clickPos = xPosToCursorPos( e->pos().x() ) - d->preeditStart;
	if ( clickPos >= 0 && clickPos <= d->preeditLength )
	    QPaintDevice::qwsDisplay()->sendIMMouseEvent( clickPos, FALSE );
...
#if !defined(QT_NO_CLIPBOARD) && defined(_WS_X11_)
    if ( c )
	copy();
#else
    Q_UNUSED(c);
...
#ifdef QT_KEYPAD_MODE //QT_KEYPAD_MODE
    // In keypad input mode, copy will copy the entire field if there is no
    // marked text.
    if ( t.isEmpty() )
	t = text();
#endif
...
QSize QLineEdit::sizeHint() const
{
    ...
    int hm = style().pixelMetric(QStyle::LineEditTextHMargin); // 2
    int vm = style().pixelMetric(QStyle::LineEditTextVMargin); // 2
    int il = style().pixelMetric(QStyle::IdealHeightLimit); // 25
    int ih = style().pixelMetric(QStyle::IdealHeight); // 22
    if ( frame() ) {
	h += vm*2 + frameW()*2;
	if ( style() == WindowsStyle && h <= il )
	    h = ih;
	return QSize( w + hm*2 + frameW()*2, h ).expandedTo( QApplication::globalStrut() );
    } else {
	return QSize( w + hm*2, h + vm*2 ).expandedTo( QApplication::globalStrut() );
    }
}
...
{ // must not call repaint() - paintEvent() calls this
	...
	static const int rightDist = 2*fm.width("x");
	static const int leftMargin = 2;
	// may need to scroll.
	QString dt = displayText();

	int leftPos = fm.width( dt.left( QMAX( cursorPos-leftMargin, 0 ) ) );
	int rightPos;

#if !defined(QT_NO_QWS_IM)
	if ( d->preeditSelLen )
	    rightPos = fm.width( dt.left( cursorPos+d->preeditSelLen+1 ) );
	else
#endif
	    rightPos = fm.width( dt.left( cursorPos ) ) + rightDist;
	
	if ( rightPos + offset > w )
	    offset = w - rightPos;
	if ( leftPos + offset < 0 )
	    offset = -leftPos;
	
	//make sure we show as much as possible of the text:

	if ( textWidth + offset < w )
	    offset = w - textWidth;
