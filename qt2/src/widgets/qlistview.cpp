/****************************************************************************
** $Id: qt/src/widgets/qlistview.cpp   2.3.12   edited 2005-10-27 $
**
...
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !listView()->isModalEditing() && !qt_singleFocusWidget(listView()) )
		...
		if ( d->useDoubleBuffer ) {
		    ...
		    dp.setBrush( p->brush() );
                    if ( testWState(WState_GlobalBrushOrigin) )
                        dp.setBrushOrigin(p->brushOrigin() - r.topLeft());
...
{
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !isModalEditing() && !qt_singleFocusWidget(this) )
...
void QListView::handleSizeChange( int section, int os, int ns )
{
    ...
    updateGeometries();
    bool fullRepaint = sx != horizontalScrollBar()->value() || sv != horizontalScrollBar()->isVisible();
    ...
    viewport()->repaint( left - d->h->cellSize( actual ), 0, d->h->cellSize( actual ),
			 visibleHeight(), FALSE );
...
#ifdef QT_KEYPAD_MODE
     else if ( qt_modalEditingEnabled && o == this ) {
	switch( e->type() ) {
...
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && e->button() == LeftButton && !isModalEditing() )
...
void QListView::contentsMouseMoveEvent( QMouseEvent * e )
{
    ...
    if ( !d->buttonDown || ( e->state() & MouseButtonMask ) == NoButton )
	return;
...
void QListView::keyPressEvent( QKeyEvent * e )
{
    ...
    if ( isMultiSelection() && i->isSelectable() && 
	    ( e->ascii() == ' ' 
#ifdef QT_KEYPAD_MODE
	      || e->key() == Key_Select
#endif
...
#endif
	d->currentPrefix.truncate( 0 );
	if ( i && (!i->isSelectable() || d->selectionMode == Single) &&
...
/*!
  ...
  if this mode should be extended or not. Extended means, that the
...
/*!
  Paints this item.
 */
void QCheckListItem::paintCell( QPainter * p, const QColorGroup & cgb,
			       int column, int width, int align )
{
    if ( !p )
	return;

    QColorGroup cg = isEnabled() ? cgb : listView()->palette().disabled();
	...
	if ( myType == CheckBox ) {
	    ...
	    p->drawRect( x+marg+1, y+3, BoxSize-6, BoxSize-6 );
	    /////////////////////
	    x++;
	    y++;
	    if ( on ) {
		...
		xx = x+2+marg;
		yy = y+5;
		a.setPoint( 0, xx, yy+1 );
		a.setPoint( 1, xx, yy+2 );
		xx++; yy++;
		for ( i=1; i<3; i++ ) {
...
#define QCOORDARRLEN(x) sizeof(x)/(sizeof(QCOORD)*2)
		y += (BoxSize-12)/2; // BoxSize allocated, 12 used, center.
...
/*!  Returns the y coordinate of the item in the list view's
  coordinate system.  This functions is normally much slower than
