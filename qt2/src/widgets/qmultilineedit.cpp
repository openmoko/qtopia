/**********************************************************************
** $Id: qt/src/widgets/qmultilineedit.cpp   2.3.12   edited 2005-10-27 $
**
...
#ifndef QT_NO_QWS_IM
#include "../kernel/qinputcontext_p.h"
#endif
#endif
...
#ifndef QT_NO_QWS_IM
    int preeditStartX, preeditStartY, preeditEndX, preeditEndY;
    int preeditCPos, preeditLength, preeditSelLen;
#endif
#ifdef QT_KEYPAD_MODE
    QString orgTxt;
    int extraNewLineCount;
    int deleteAllTimerId;
#endif
...
#ifndef QT_NO_QWS_IM
bool QMultiLineEdit::composeMode() const
{
    return  d->preeditStartY >= 0;
}
#endif

static bool inBlinkTimer = FALSE;

#ifndef QT_NO_QWS_IM
extern QBrush *qt_im_compose_background;
#endif
    ...
    :QTableView( parent, name, WNorthWestGravity | WRepaintNoErase )
{
	...
	setBackgroundMode( PaletteBackground ); // Until modal
    } else
#else
...
*/
void QMultiLineEdit::paintCell( QPainter *painter, int row, int col )
{
	...
	rowYPos(row, &cy);
	colXPos(col, &cx);
	pbuf.setBrushOrigin(painter->brushOrigin() - updateR.topLeft() - QPoint(cx, cy));
    }
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !isModalEditing() )
	pbuf.fillRect( 0, 0, buffer->width(), buffer->height(), g.brush(QColorGroup::Background) );
    else
#endif
    pbuf.fillRect( 0, 0, buffer->width(), buffer->height(), g.brush(QColorGroup::Base) );
	...
	if ( row >= d->preeditStartY && row <= d->preeditEndY ) {
	    int sLength = s.length();
	    int preX1 = (row == d->preeditStartY) ? d->preeditStartX : 0;
	    int preX2 = (row == d->preeditEndY) ? d->preeditEndX : sLength;

	    int prexpos1   =  mapToView( preX1, row );
	    int prexpos2   =  mapToView( preX2, row );

	    if ( !qt_im_compose_background ) {
		p.setPen( g.text() );
		p.drawLine( prexpos1, cellHeight()-2, prexpos2, cellHeight()-2 );
	    } else {
		if ( markX1 == 0 )
		    prexpos1 -= 2;
		if ( markX2 == sLength )
		...
		p.fillRect( prexpos1, 0, prexpos2 - prexpos1, cellHeight(row),
			     *qt_im_compose_background );
	    }	     
	}
	//### abusing mark anchor
	if ( ( markAnchorY < markDragY || 
	     markAnchorY == markDragY && markAnchorX < markDragX)
	     && row >= markAnchorY && row <= markDragY) {
	    markX1 = row == markAnchorY ? markAnchorX : 0;
	    markX2 = row == markDragY ? markDragX : s.length();
	}
    }
#endif //QT_NO_QWS_IM
    p.setPen( g.text() );
...
#ifdef QT_KEYPAD_MODE
			    && (!qt_modalEditingEnabled || isModalEditing() ) 
#endif
			    )
     {
...
#else
	    int xxpos,yypos;
	    if ( !inBlinkTimer && colXPos( 0, &xxpos ) && rowYPos( row, &yypos ) )
		setMicroFocusHint( cXPos+xxpos,
				   cYPos+yypos,
				   1, fm.height() );
#endif
	}
...
void QMultiLineEdit::keyPressEvent( QKeyEvent *e )
{
    ...
    if( qt_modalEditingEnabled ) {
	switch( e->key() ) {
	    // two enters in a row to insert a return, one enter followed by any
	    // other key to escape from modal mode
	    case Key_Select:
		setModalEditing( !isModalEditing() );
		return;
	    case Key_Back:
	    case Key_No:
		if ( !isModalEditing() ) {
	...
	case Key_Back:
	    if (!e->isAutoRepeat()) {
		if ( qt_modalEditingEnabled && !length() ) {
		    setText(d->orgTxt);
		    setEdited(FALSE);
		    setModalEditing( FALSE );
		} else if ( qt_modalEditingEnabled && d->deleteAllTimerId < 0) {
		    d->deleteAllTimerId = startTimer(750);
...
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled ) {
	if (cursorY == numLines()-1 && lineLength(numLines()-1) != 0)
	    d->extraNewLineCount = 0;
    }
#endif
...
{
    // don't check for compose end.  this is used by compose events as well.
    dummy = FALSE;
    d->blinkTimer->stop();
    cursorOn = TRUE;
...
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled ) {
	if (d->extraNewLineCount && cursorY == (int)contents->count() - 2
	    && lineLength(numLines()-1) == 0) {
	    del();
	    d->extraNewLineCount--;
	}
    }
...
{
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && d->extraNewLineCount && cursorY == numLines() - 1
	&& lineLength(numLines()-1) == 0 && !mark) {
	backspace();  // this will decrement d->extraNewLineCount
	home(FALSE);
    } else
#endif
    if ( cursorY != 0 ) {
...
void QMultiLineEdit::cursorDown( bool mark, bool clear_mark )
{
	...
	if ( cursorY == lastLin ) {
	    ...
	    insertAt( "\n", numLines()-1, lineLength(numLines()-1), mark );
	    d->extraNewLineCount++;
	    lastLin = contents->count() - 1;
	}
    }
#endif
    if ( cursorY != lastLin ) {
...
void	QMultiLineEdit::removeText( int markBeginY, int markBeginX,
				    int markEndY, int markEndX )
{
...
void QMultiLineEdit::delAux()
{
	...
	removeText( markBeginY, markBeginX, markEndY, markEndX );
...
#ifndef QT_NO_QWS_IM
int  QMultiLineEdit::pixelPosToIMPos( const QPoint &p ) const
{
    int xPos, yPos;
    pixelPosToCursorPos( p, &xPos, &yPos );

    //count to find the position
    int clickPos = 0;
    if ( yPos > d->preeditEndY || 
	 yPos == d->preeditEndY && xPos > d->preeditEndX ) {
	xPos = d->preeditEndX;
	yPos = d->preeditEndY;
    }

	
    int x = d->preeditStartX;
    int y = d->preeditStartY;
...
void QMultiLineEdit::mousePressEvent( QMouseEvent *e )
{
    ...
    if ( composeMode() ) {
#if defined (_WS_QWS_)
	QPaintDevice::qwsDisplay()->sendIMMouseEvent( pixelPosToIMPos( e->pos() ), TRUE ); 
...
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled ) {
	if ( e->button() == LeftButton && !isModalEditing() )
	    setModalEditing( TRUE );
    }
#endif
...
void QMultiLineEdit::pixelPosToCursorPos(QPoint p, int* x, int* y) const
{
    ...
    *x = xPosToCursorPos( stringShown( *y ), fm,
			  ...
			  d->align );
    QMultiLineEditRow* r = contents->at( *y );
...
{
    int newX, newY;
    pixelPosToCursorPos( p, &newX, &newY );
#ifdef QT_KEYPAD_MODE
    if( qt_modalEditingEnabled ) {
	while (d->extraNewLineCount > 0 && cursorY == numLines() - 1
	    && lineLength(cursorY) == 0 && cursorX == 0 && cursorY > newY) {
	    backspace();
	}
    }
#endif
...
void QMultiLineEdit::mouseReleaseEvent( QMouseEvent *e )
{
    ...
    if ( composeMode() ) {
#if defined (_WS_QWS_)
	QPaintDevice::qwsDisplay()->sendIMMouseEvent( pixelPosToIMPos( e->pos() ), FALSE ); 
...
void QMultiLineEdit::clear()
{
    ...
    QString composeText;
    if (composeMode()) {
	/* if there is compose text, don't throw it away. */
	for (int ind = d->preeditStartY; ind <= d->preeditEndY; ++ind) {
	    QString s = textLine(ind);
	    int cstart = ind == d->preeditStartY ? d->preeditStartX: 0;
	    int cend = (ind == d->preeditEndY ? d->preeditEndX : s.length()-1);
	    composeText += s.mid(cstart, cend-cstart);
	}
    }
#endif
...
#ifndef QT_NO_QWS_IM
    /* if was in compose mode */
    if (composeMode()) {
	d->preeditStartX = 0;
	d->preeditStartY = 0;
	if (!composeText.isNull() > 0) {
	    // in im compose, text is actually cleared...
	    insertAtAux( composeText, 0, 0, FALSE);
	    recalcIM();
	}
    }
#endif
...
#ifdef QT_KEYPAD_MODE
    // In keypad mode, copy will copy the entire field if there is no
    // marked text.
    if ( t.isEmpty() )
	t = text();
#endif
...
{
#if defined(_WS_X11_)
    disconnect( (QObject*)(QApplication::clipboard()), SIGNAL(dataChanged()),
		this, SLOT(clipboardChanged()) );
    turnMark( FALSE );
    update();
#endif
...
void QMultiLineEdit::setCursorPosition( int line, int col, bool mark )
{
    ...
    if( qt_modalEditingEnabled ) {
	while (d->extraNewLineCount > 0 && cursorY == numLines() - 1
	    && lineLength(cursorY) == 0 && cursorX == 0 && cursorY > line) {
	    backspace();
	}
    }
#endif
...
void QMultiLineEdit::resizeEvent( QResizeEvent *e )
{
	...
	bool oldAuto = autoUpdate();
	setAutoUpdate( FALSE );
#if !defined(QT_NO_QWS_IM)
	if ( composeMode() ) {
	    //Slightly evil hack part I
	    cursorY = d->preeditStartY;
	    cursorX = d->preeditStartX;
	}
#endif
	rebreakAll();
...
#if !defined(QT_NO_QWS_IM)
	if ( composeMode() ) {
	    //Slightly evil hack part II
	    d->preeditStartY = cursorY;
	    d->preeditStartX = cursorX;
	    recalcIM();
	}
#endif
	setAutoUpdate( oldAuto );
...
void QMultiLineEdit::wrapLine( int line, int removed )
{
	...
	if ( s[i] == '\t' && d->align == Qt::AlignLeft ) {
	    ...
	    unsigned char c = (unsigned char)s[i].latin1();
	    if ( c > 0 ) {
		if ( !d->chartable[c] )
	...
	if ( sh && yPos >= contentsRect().top() && yPos < contentsRect().bottom() ) {
		...
		if ( rowYPos( d->maxlines-1, &maxy ) ) {
            ...
            if (testWState(WState_GlobalBrushOrigin)) {
                QRect r1(contentsRect().left(), yPos, contentsRect().width(), h);
                QRect r2(r1);
                r2.moveBy(0, sh);
                repaint(r1 | r2);
            } else {
...
void QMultiLineEdit::blinkTimerTimeout()
{
    ...
    int cursorPos = QMIN( (int)s.length(), cursorX );
    int cXPos   = QMAX( mapToView( cursorPos, cursorY ), 0 );
    int xPos, yPos;
    if ( colXPos( 0, &xPos ) && rowYPos( cursorY, &yPos ) ) {
	//QRect uR = QRect( xPos, yPos, cellWidth(0), cellHeight(cursorY) );
	QRect uR = QRect( cXPos, yPos, 1, cellHeight(cursorY) );
...
bool QMultiLineEdit::event( QEvent * e )
{
...
#endif //QT_NO_QWS_IM
#ifdef QT_KEYPAD_MODE
    case QEvent::EnterModalEdit:
	if( qt_modalEditingEnabled ) {
	    setBackgroundMode( PaletteBase );
	    d->orgTxt = text();
	}
	break;
    case QEvent::LeaveModalEdit:
	if( qt_modalEditingEnabled ) {
    ...
    case QEvent::KeyRelease: {
	 QKeyEvent *ke = (QKeyEvent *)e;
	 if ( !ke->isAutoRepeat() && ke->key() == Key_Back &&
	      d->deleteAllTimerId > 0) {
	     killTimer(d->deleteAllTimerId);
	     d->deleteAllTimerId = -1;
	     backspace();
	 }
     }
     break;
	...
	if ( te->timerId() == d->deleteAllTimerId ) {
	    killTimer(d->deleteAllTimerId);
	    d->deleteAllTimerId = -1;
	    clear();
	}
...
#ifndef QT_NO_QWS_IM
void QMultiLineEdit::imStartEvent( QIMEvent *e )
{
    ...
    */
    ...
    insert(""); //remove marked text, if any
    d->preeditStartX = cursorX;
    d->preeditStartY = cursorY;
    d->preeditEndY = -1;
    d->preeditEndX = -1;
...
void QMultiLineEdit::imComposeEvent( QIMEvent *e )
{
    if (isReadOnly())
	return;
    bool updating = autoUpdate();
    if ( updating )
	setAutoUpdate( FALSE );

    /* why is this done? */
    if ( d->preeditEndY >= d->preeditStartY ) {
	removeText( d->preeditStartY, d->preeditStartX,
		    d->preeditEndY, d->preeditEndX );
    }

    
    //need to calculate endY and cpos and so on...
    //we could make insert calculate that if we're in compose mode

    d->preeditLength = e->text().length();
    d->preeditCPos = e->cursorPos();
    d->preeditSelLen = e->selectionLength();
    
    
    //insert() is undo-aware; we must use raw insertion:
    insertAtAux( e->text(), d->preeditStartY, d->preeditStartX, FALSE );
...
/*
  Inputs (all from QMultiLineData):

  preeditStartX, preeditStartY, preeditLength, preeditCPos, preeditSelLen

  Calculates the other positions needed in compose mode.

*/
void QMultiLineEdit::recalcIM()
{

    //First pretend we are working on a single line.
    d->preeditEndY = d->preeditStartY;
    d->preeditEndX = d->preeditStartX + d->preeditLength;

    cursorY = d->preeditStartY;
    cursorX = d->preeditStartX + d->preeditCPos;
    curXPos = 0;
    markAnchorX = cursorX;
    markAnchorY = cursorY;
    markDragX = markAnchorX + d->preeditSelLen;
    markDragY = markAnchorY;


    // Count up and find out where the various positions in the string
    // ended up in the editor.

    int nStart = d->preeditStartX;
    int nPre = d->preeditLength + d->preeditStartX;
    int nCur = QMIN( d->preeditCPos + d->preeditStartX, nPre ); 
    int nSel = QMIN( nCur + d->preeditSelLen, nPre ); 
    int row = d->preeditStartY;
    while ( nPre > 0 ) {
	QMultiLineEditRow *r = contents->at( row );
	if (!r) {
	    // was truncated at end of text... preditLength not valid
	    if (nStart >= 0) {
		d->preeditStartY = numLines()-1;
		d->preeditStartX = lineLength(d->preeditStartY);
		d->preeditEndX = d->preeditStartX;
		d->preeditEndY = d->preeditStartY;
		d->preeditCPos = 0;
		d->preeditSelLen = 0;
		d->preeditLength = 0;
	    } else {
		// set preeditLength,Cpos,Sellen
		// nPre is text left to process?
		d->preeditEndY = numLines()-1;
		d->preeditEndX = lineLength(d->preeditStartY);
		// nPre is where end would be if it was on this line..
		// hence in this case, it is also how much we over shot by
		d->preeditLength -= nPre;
		if (nCur >= 0) {
		    cursorY = d->preeditEndY;
		    cursorX = d->preeditEndX;
		    markAnchorX = cursorX;
		    markAnchorY = cursorY;
		    d->preeditCPos -= nCur;
		}
		if (nSel >= 0) {
		    markDragY = d->preeditEndY;
		    markDragX = d->preeditEndX;
		    d->preeditSelLen -= nSel;
		}
	    }
	    break;
	}
	
	if ( nStart > (int)r->s.length() ) {
	    nStart -= r->s.length();
	} else if ( nStart >= 0 ) {
	    d->preeditStartY = row;
	    d->preeditStartX = nStart;

	    nStart = -1;
	}

	if ( nCur > (int)r->s.length() || 
	     nCur == (int)r->s.length() && nCur < nPre) { //eol is start of next
	    nCur -= r->s.length();
	} else if ( nCur >= 0 ) {
	    cursorY = row;
	    cursorX = nCur;
...
void QMultiLineEdit::forceIMEnd()
{
    if (composeMode()) {
	bool updating = autoUpdate();
	if ( updating )
...
#ifdef _WS_QWS_
	QInputContext::notifyWidgetForcedIMEnd(this);
#endif
    }
}

void QMultiLineEdit::imEndEvent( QIMEvent *e )
{
    if (isReadOnly()) {
	bool updating = autoUpdate();
	if ( updating )
    ...
    bool updating = composeMode() && autoUpdate();
    if ( updating )
	setAutoUpdate( FALSE );
...
bool QMultiLineEdit::focusNextPrevChild( bool b)
{
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !isModalEditing() )
	return QWidget::focusNextPrevChild( b );
    else
#else
    Q_UNUSED( b );
#endif
