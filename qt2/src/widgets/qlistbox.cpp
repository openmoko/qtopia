/**********************************************************************
** $Id: qt/src/widgets/qlistbox.cpp   2.3.12   edited 2005-10-27 $
**
...
void QListBox::focusInEvent( QFocusEvent *e )
{
    ...
    if ( 
#ifdef QT_KEYPAD_MODE
    !qt_modalEditingEnabled &&
#endif
	    e->reason() != QFocusEvent::Mouse && !d->current && d->head ) {
	setCurrentItem(d->head);
    } else if ( d->current )
...
/*!
  ...
  FALSE, clearSelection() is called if \a item is the currently
  selected item.
...
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && !isModalEditing() && !qt_singleFocusWidget(this) )
	p.fillRect( 0, 0, w, h, g.brush( QColorGroup::Background ) );
    else
#endif
...
void QListBox::paintCell( QPainter * p, int row, int col )
{
	...
	style().drawFocusRect( p, QRect( 0, 0, cw, ch ),
			       g, i->selected() ? &g.highlight() : &bg.color(),
			       TRUE );
    }
