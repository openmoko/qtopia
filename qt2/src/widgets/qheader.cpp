/****************************************************************************
** $Id: qt/src/widgets/qheader.cpp   2.3.12   edited 2005-10-27 $
**
...
/*!
  Appends or removes (from the end) sections so that the total
  number equals /a n. New sections get a default label.

  Faster than calling addLabel()/removeLabel() in a loop.
 */
void QHeader::setNumLabels( int n )
{
    if ( n > d->count ) {
	// add labels
	...
	for ( int section = d->count; section < n; section++ ) {
	    d->labels.insert( section, new QString( QString::number(section) ) );
	    /*
	      int iw = 0;
	      int ih = 0;
	...
	qWarning( "not really implemented yet" );
	while ( n < d->count )
	    removeLabel( d->count - 1 );
...
/*!
  Returns the section which contains the position \a pos given in pixels.
