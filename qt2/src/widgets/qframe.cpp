/****************************************************************************
** $Id: qt/src/widgets/qframe.cpp   2.3.12   edited 2005-10-27 $
**
...
/*!
    \Internal
    Needed to stay binary compatible
*/
class QFrameEventHandler : public QObject
{
public:
    QFrameEventHandler(QWidget *parent)
	: QObject(parent, 0)
    {
    ...
    {
	return ((QFrame *)o)->eventPrivate(e);
    }
};
#endif
    ...
    : QWidget( parent, name, f )
{
	...
	(void) new QFrameEventHandler(this);
#endif
}
...
#ifdef QT_KEYPAD_MODE
    if ( qt_modalEditingEnabled && hasFocus() && !isModalEditing())
        style().drawFocusRect(p, frameRect(), g, &g.highlight(), TRUE);
#endif
}
...
/*!
    \Internal
*/
bool QFrame::eventPrivate(QEvent *e)
{
