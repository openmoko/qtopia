/**********************************************************************
** $Id: qt/src/widgets/qtableview.cpp   2.3.12   edited 2005-10-27 $
**
...
void QTableView::coverCornerSquare( bool enable )
{
	...
	cornerSquare->setGeometry( maxViewX() + 1,
				   maxViewY() + 1,
                                   VSBEXT,
                                 HSBEXT);
    }
...
void QTableView::paintEvent( QPaintEvent *e )
{
	...
	paint.save();
	QRegion cr(updateR.intersect(frameRect()));
	cr -= contentsRect();
	...
	if ( inherits( "QMultiLineEdit" ) ){
	    ...
	    if (!hasFocus()){
		// clear the frame and margin
		QBrush brush(colorGroup().brush(QColorGroup::Base));
#ifdef QT_KEYPAD_MODE
		if( qt_modalEditingEnabled )
		    brush = colorGroup().brush(QColorGroup::Background);
#endif
...
#ifdef QT_KEYPAD_MODE
    if (qt_modalEditingEnabled && isModalEditing()){
	paint.fillRect(maxWX+1+margin(), 1, width(), height(), colorGroup().brush(QColorGroup::Background)); // fill gap around scrollbars 
	paint.fillRect(lineWidth(), maxWY+1+margin(), width()-2*lineWidth(), height() - maxWY, colorGroup().brush(QColorGroup::Background));  
    }
#endif
...
#ifdef QT_KEYPAD_MODE
	    if ( qt_modalEditingEnabled && !isModalEditing() )
		paint.fillRect( updateR, g.brush(QColorGroup::Background) );
	    else
#endif
	    paint.fillRect( updateR, g.brush(QColorGroup::Base) );
	} else {
	    paint.eraseRect( updateR );
	}
	return;
...
void QTableView::updateScrollBars( uint f )
{
	...
	if ( sbDirty & verGeometry )
	    vScrollBar->setGeometry( width() - VSBEXT, 0,
                                     VSBEXT, viewHeight()+2*frameWidth());
	...
	cornerSquare->move( maxViewX() + 1 + frameWidth(),
			    maxViewY() + 1 + frameWidth());
