/****************************************************************************
** $Id: qt/src/widgets/qtextview.cpp   2.3.12   edited 2005-10-27 $
**
...
/*!
  ...
  QStyleSheet for details. Currently, a small XML/CSS1 subset including
  embedded images and tables is supported. See QStyleSheet for
...
/*!
  ...
  For rich text the rendering style and available tags are defined by
  a styleSheet(); see QStyleSheet for details.
...
void QTextView::createRichText()
{
    ...
    d->doc_ = new QRichText( d->txt, viewport()->font(), d->contxt,
			     offset, mimeSourceFactory(), styleSheet() );
...
const QBrush& QTextView::paper()
{
	...
	if ( !isModalEditing() && !qt_singleFocusWidget(this) )
	    return d->papcolgrp.brush( QColorGroup::Background );
    }
#endif
				 ...
				 int cx, int cy, int cw, int ch)
{
    ...
    if ( paper().pixmap() && !testWState(WState_GlobalBrushOrigin) )
...
void QTextView::updateLayout()
{
    ...
    // This condition is wrong. See Qtopia bug 2271 (among others).
