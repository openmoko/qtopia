/****************************************************************************
** $Id: qt/src/widgets/qcommonstyle.cpp   2.3.12   edited 2005-10-27 $
**
...
void QStyle::drawToolButton( QToolButton* btn, QPainter *p)
{
	...
	drawToolButton( p, x, y, w, h, g, sunken, &fill );
	      ...
	      !btn->parentWidget()->backgroundPixmap()->isNull() ) {
	if ( btn->testWState(WState_GlobalBrushOrigin) && btn->parentWidget()->backgroundMode() != QWidget::FixedPixmap ) {
	    // paletted brush background, we need to use the brush origin
	    p->fillRect( 0, 0, btn->width(), btn->height(),
		QBrush(QColor(),*btn->parentWidget()->backgroundPixmap()) );
	} else {
	    p->drawTiledPixmap( 0, 0, btn->width(), btn->height(),
				*btn->parentWidget()->backgroundPixmap(),
				btn->x(), btn->y() );
