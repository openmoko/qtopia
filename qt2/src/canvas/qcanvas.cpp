/**********************************************************************
** $Id: qt/src/canvas/qcanvas.cpp   2.3.12   edited 2005-10-27 $
**
...
/*!  Sets the QCanvas to be composed of \a h tiles horizontally and \a
  ...
  The width and height of \a p must be multiples of \a tilewidth and
...
/*!
  Sets the array to \a fc pixmaps, read from files.

  If \a fc is not 0, the
...
If \a i is more than count()-1, the array is extended to make room.
*/
...
{
    if ( viewing ) {
	if ( viewing->width() < contentsWidth() ) {
	    QRect r(contentsToViewport(QPoint(viewing->width(),0)),
		    QSize(contentsWidth()-viewing->width(),contentsHeight()));
	    viewport()->erase(r);
	}
	if ( viewing->height() < contentsHeight() ) {
	    QRect r(contentsToViewport(QPoint(0,viewing->height())),
		    QSize(contentsWidth(),contentsHeight()-viewing->height()));
	    viewport()->erase(r);
	}
...
{
    QPointArray pa = areaPoints().copy();
...
{
    QFontMetrics fm(fnt);
    brect = fm.boundingRect(int(x()), int(y()), 0, 0, flags, txt);
}
