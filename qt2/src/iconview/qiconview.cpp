/****************************************************************************
** $Id: qt/src/iconview/qiconview.cpp   2.3.12   edited 2005-10-27 $
**
...
void QIconView::arrangeItemsInGrid( bool update )
{
	...
	if( !isAgain ) { isAgain = TRUE; arrangeItemsInGrid( FALSE ); }
    isAgain = FALSE;
    viewport()->setUpdatesEnabled( TRUE );
    d->dirty = FALSE;
    rebuildContainers();
...
QSize QIconView::sizeHint() const
{
    ...
    int b = 2*frameWidth();
    return QSize( QMIN( 400, contentsWidth() + style().scrollBarExtent().width() + b),
		  QMIN( 400, contentsHeight() + style().scrollBarExtent().height() + b ) );
}
