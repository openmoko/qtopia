/****************************************************************************
** $Id: qt/src/tools/qglobal.cpp   2.3.12   edited 2005-10-27 $
**
** Global functions
**
** Created : 920604
...
#define generateMsgFunction(name,type) \
void name(const char *msg, ...) \
{ \
    va_list ap; \
    va_start(ap, msg); \
    qtMessageOutput(type, msg, ap); \
    va_end(ap); \
}


// Avoid errors from copy and pasted code 
generateMsgFunction(qDebug,QtDebugMsg)
generateMsgFunction(debug,QtDebugMsg)
generateMsgFunction(qWarning,QtWarningMsg)
generateMsgFunction(warning,QtWarningMsg)
generateMsgFunction(qFatal,QtFatalMsg)
generateMsgFunction(fatal,QtFatalMsg)
