/****************************************************************************
** $Id: qt/src/dialogs/qmessagebox.cpp   2.3.12   edited 2005-10-27 $
**
...
#if defined(_WS_QWS_)
    // small buttons hard to hit with stylus
    maxSize.setWidth( QMAX(maxSize.width(), 45) );
#endif
...
#if defined(_WS_QWS_)
	// check displaysize, if smaller than say, 8 times current image width,
	// than the 'icon' should be smaller as the text is more important.
	extern Q_EXPORT QRect qt_maxWindowRect;
	if ((image.width() << 3) > qt_maxWindowRect.width()) {
	    image = image.smoothScale(qt_maxWindowRect.width() >> 3, qt_maxWindowRect.width() >> 3);
	}
#endif
...
void QMessageBox::adjustSize()
{
	...
	border = 10;
#else
    // This is just dumb.  On a small screen this wastes a huge amount
    // of space.
    int border = bh/3 - style().buttonDefaultIndicatorWidth();
    if ( border <= 0 )
	border = 6;
#endif
    int btn_spacing = 7;
