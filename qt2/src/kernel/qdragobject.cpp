/****************************************************************************
** $Id: qt/src/kernel/qdragobject.cpp   2.3.12   edited 2005-10-27 $
**
...
#else
		str = QCString(e->encodedData(QCString(mime)));
		return TRUE;
#endif
	    }
	}
...
// Cache most recently used objects for quick 'decoding',
// bypassing serialization through PNG completely.
//
// QImageDrag objects are short lived. "1" would probably do
// for this. Certainly no more than 4 - especially since the
// array is searched linearly.
static const int imgdrag_cache_max = 4;
//
static QImageDrag* imgdrag_cache[imgdrag_cache_max];
//
static int imgdrag_cache_lru = -1;
//
static void imgdrag_cache_in(QImageDrag* id)
{
    if ( imgdrag_cache_lru < 0 ) {
	// Init
	imgdrag_cache_lru = 0;
	memset(imgdrag_cache,0,imgdrag_cache_max*sizeof(imgdrag_cache[0]));
    }
    imgdrag_cache[imgdrag_cache_lru] = id;
    imgdrag_cache_lru = (imgdrag_cache_lru + 1)%imgdrag_cache_max;
}
static void imgdrag_cache_out(QImageDrag* id)
{
    for (int i=0; i<imgdrag_cache_max; i++) {
	if ( imgdrag_cache[i] == id ) {
	    imgdrag_cache[i] = 0;
	    break;
	}
    }
}
static const QImageDrag* imgdrag_cache_find(const QMimeSource* s)
{
    for (int i=0; i<imgdrag_cache_max; i++) {
	if ( imgdrag_cache[i] == s )
	    return imgdrag_cache[i];
    }
    return 0;
}
...
{
    // Short-circuit: Look for created QImageDrag.
    const QImageDrag* c = imgdrag_cache_find(e);
    if ( c ) {
	img = c->img;
	if ( !img.isNull() )
	    return TRUE;
