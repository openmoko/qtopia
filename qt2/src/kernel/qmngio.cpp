/****************************************************************************
** $Id: qt/src/kernel/qmngio.cpp   2.3.12   edited 2005-10-27 $
**
...
int g_qmng_loop_hack = -1; /* Can't determine number of loops
			       using libmng. Applications (ie. Qtopia)
			       can set this value so the number of 
			       loops for MNG is enforced. 
				     */
...
public:
    ...
    bool openstream()
    {
	// ### We should figure out from libmng how many loops an MNG has(if possible), but for now it's internally specified.
	if (consumer) {
	    int loop = g_qmng_loop_hack;
	    if( loop < 0 )
		loop = 0;
