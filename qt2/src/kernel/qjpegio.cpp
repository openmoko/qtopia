/****************************************************************************
** $Id: qt/src/kernel/qjpegio.cpp   2.3.12   edited 2005-10-27 $
**
...
public:
	...
	cacheHeight = 1;
	imageCache.create(info->output_width, cacheHeight, 32);
    }
...
private:
    ...
    QRgb*
    scanLine(const int line = 0, const QImage *src = 0)
    {
	QRgb    *out;
	uchar	*in;
	...
	jpeg_read_scanlines(cinfo, imageCache.jumpTable(), 1);
...
static QImage
qJpegHeaderInformation(struct jpeg_decompress_struct *cinfo)
{
    if (cinfo->output_components == 3 || cinfo->output_components == 4) {
	return QImage(NULL, cinfo->output_width, cinfo->output_height, 32,
	    NULL, 0, QImage::IgnoreEndian);
    } else if (cinfo->output_components == 1) {
	return QImage(NULL, cinfo->output_width, cinfo->output_height, 8,
	    NULL, 0, QImage::IgnoreEndian);
    }
