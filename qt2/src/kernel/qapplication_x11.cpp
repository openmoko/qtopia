/****************************************************************************
** $Id: qt/src/kernel/qapplication_x11.cpp   2.3.12   edited 2005-10-27 $
**
...
#include <sys/ioctl.h>
#ifdef __CYGWIN__
#include <sys/termios.h> // needed to define FIONREAD
#endif
...
#  if defined(_OS_UNIX_)
    char c = 0;
#   ifndef __CYGWIN__
	const int FIONREAD_VALUE = FIONREAD;
#   else
	const int FIONREAD_VALUE = TIOCINQ;	    
#   endif
    int nbytes;
    if ( ::ioctl(qt_thread_pipe[0], FIONREAD_VALUE, (char*)&nbytes) >= 0 && nbytes == 0 ) {
					   ...
					   char& ascii, int& code )
{
	...
	state |= Keypad;
    } else if ( key >= XK_Multi_key && key <= XK_PreviousCandidate) {
	code = key - 0xee00; //international input method support
    } else {
