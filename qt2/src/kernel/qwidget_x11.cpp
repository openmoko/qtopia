/****************************************************************************
** $Id: qt/src/kernel/qwidget_x11.cpp   2.3.12   edited 2005-10-27 $
**
			...
			bool showIt )
{
    ...
    if ( isTopLevel() && extra && extra->topextra && extra->topextra->xic ) {
    ...
    // recreate input context if needed
    if ( isTopLevel() ) {
        // if we are reparenting back to toplevel,
...
void QWidget::checkChildrenDnd()
{
	...
	widget->createExtra();
	widget->extraData()->children_use_dnd = children_use_dnd;
