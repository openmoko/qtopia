/****************************************************************************
** $Id: qt/src/kernel/qmovie.cpp   2.3.12   edited 2005-10-27 $
**
						...
						const QRect& area)
{
    ...
    // scale it if neccessary 
    if( area.width() != img.width() ||
	    area.height() != img.height () )
	img = img.smoothScale( area.width(), area.height() );
	...
	p.begin(&mypixmap);
	p.fillRect(area, bg);
	p.drawPixmap(area.x(), area.y(), lines);
	p.end();
    } else {
...
QMovie::QMovie(const QString &fileName, int bufsize)
{
    ...
    d = new QMovieFilePrivate(new QIODeviceSource(file, bufsize), this, bufsize);
}
...
QMovie::QMovie(QByteArray data, int bufsize)
{
    ...
    d = new QMovieFilePrivate(new QIODeviceSource(buffer, bufsize), this, bufsize);
}
