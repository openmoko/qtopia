/****************************************************************************
** $Id: qt/src/kernel/qstyle.cpp   2.3.12   edited 2005-10-27 $
**
...
public:
	...
	:sbextent(16,16),
	 ...
	 slider_thickness(16)
    {
        pm[QStyle::ComboBoxFocusMargin] = 1;
        pm[QStyle::ComboBoxTextHMargin] = 2;
        pm[QStyle::ComboBoxTextVMargin] = 1;
        pm[QStyle::ComboBoxTextVMarginExtra] = 1;
        pm[QStyle::LineEditTextHMargin] = 2;
        pm[QStyle::LineEditTextVMargin] = 2;

        pm[QStyle::SpinBoxButtonScale] = 1;
        pm[QStyle::SpinBoxHMargin] = 6;
        pm[QStyle::SpinBoxVMargin] = 4;
        pm[QStyle::SpinBoxHExtraScale] = 1;

        pm[QStyle::IdealHeightLimit] = 25;
        pm[QStyle::IdealHeight] = 22;
	pm[QStyle::CheckBoxGap] = 6;
	pm[QStyle::TabHMargin] = 6;

	// default constructor (invalid) means don't use
	ec[QStyle::AlternateBase] = QBrush();
    }
    ...
    int pm[(int)QStyle::LastPixelMetric];
    QBrush ec[(int)QStyle::LastExtendedColor];
};
...
{
    QSize s =  d(this)->sbextent;
#if defined(QT_KEYPAD_MODE)
    if( qt_modalEditingEnabled )
#endif
    // don't expand on touchscreen - desired height set explicitly by theme
...
{
    if ( pixelmetric < LastPixelMetric )
	return d(this)->pm[pixelmetric];
    return 0;
}
...
/*!
  ...
  Not available prior to Qt 2.3.8.
*/
void QStyle::setPixelMetric(PixelMetric pixelmetric, int value)
{
    if ( pixelmetric < LastPixelMetric )
	d(this)->pm[pixelmetric] = value;
}
...
{
    if ( extendedcolor < LastExtendedColor )
	return d(this)->ec[extendedcolor];
    return QBrush();
}
