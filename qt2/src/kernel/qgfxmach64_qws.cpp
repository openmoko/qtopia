/****************************************************************************
** $Id: qt/src/kernel/qgfxmach64_qws.cpp   2.3.12   edited 2005-10-27 $
**
...
{
    qDebug( "QGfxMach64::drawLine()" );
    // No point going any further if the window isn't visible
    if(this->ncliprect<1)
        return;

    // Only handle 'normal' lines
    if ( this->cpen.style() != this->SolidLine || this->myrop!=this->CopyROP ) {
    ...
    wait_for_fifo(1);
    regw(DP_FRGD_CLR,this->cpen.color().pixel());
    ...
    qDebug( "cliprect[%d](%d,%d,%d,%d)", loopc, this->cliprect[loopc].x(), this->cliprect[loopc].y(),
	    this->cliprect[loopc].right(), this->cliprect[loopc].bottom() );
...
void QGfxMach64<depth,type>::fillRect(int rx,int ry,int w,int h)
{
    ...
    if( (this->cbrush.style()!=this->NoBrush) && (this->cbrush.style()!=this->SolidPattern) ) {
	...
	regw(SC_LEFT,0);
	regw(SC_TOP,0);
	regw(SC_RIGHT,this->width);
	regw(SC_BOTTOM,this->height);
...
inline void QGfxMach64<depth,type>::blt(int rx,int ry,int w,int h,int sx, int sy)
{
    ...
    if(this->alphatype==QGfx::BigEndianMask || this->alphatype==QGfx::LittleEndianMask ||
       this->alphatype==QGfx::SeparateAlpha || this->srctype==QGfx::SourcePen || (this->myrop!=this->CopyROP) ) {
					...
					int sw,int sh)
{
	...
	if (!qt_screen->onCard(this->srcbits,my_src_buffer_offset))
				       ...
				       int x3,int y3,int x4,int y4)
{
	...
	if (!qt_screen->onCard(this->srcbits,foffset))
	    qFatal("checkSourceDest() lied!");
