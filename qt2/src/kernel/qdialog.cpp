/****************************************************************************
** $Id: qt/src/kernel/qdialog.cpp   2.3.12   edited 2005-10-27 $
**
...
QSize QDialog::sizeHint() const
{
    ...
    QSize ds = qt_maxWindowRect.size();
    QSize extraSize( frameGeometry().width() - geometry().width(),
		     frameGeometry().height() - geometry().height() );
    extraSize = extraSize.boundedTo( QSize(10, 40) );
    ds.setWidth( ds.width() - extraSize.width() );
    ds.setHeight( ds.height() - extraSize.height() );
    if ( s.width() > ds.width() || s.height() > ds.height() ) {
	s = s.boundedTo( ds ).expandedTo( minimumSizeHint() );
    }
