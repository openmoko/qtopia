/****************************************************************************
** $Id: qt/src/kernel/qpainter.cpp   2.3.12   edited 2005-10-27 $
**
...
bool QPainter::begin( const QPaintDevice *pd, const QWidget *copyAttributes )
{
	...
	if ( copyAttributes->testWState(WState_GlobalBrushOrigin) )
	    setBrushOrigin( -copyAttributes->mapToGlobal( copyAttributes->rect().topLeft() ) );
	return TRUE;
    }
    return FALSE;
}
...
const int TxTranslate = 1;			// copy in qpainter_*.cpp
const int TxScale     = 2;
		     ...
		     char **internal, QPainter* painter )
{
    ...
    const uint ZWSP     = 0x10000000;   // zero-width space (no-op)
    const uint HYPH     = 0x90000000;   // BEGLINE+ZWSP is hyphen line
    ...
    const int WIDTHBITS= 0x0fffffff;	// bits for width encoding
    const int MAXWIDTH = 0x0fffffff;	// max width value
...
#define ISPRINT(x) ((x).row() || (x).cell()>' ')
    ...
    static const QChar shy(0x00ad); // soft hyphen

    int	 spacewidth = CWIDTH( QChar(' ') );	// width of space char
    int	 shywidth = CWIDTH( QChar('-') );		// width of hyphen
	...
	if ( !fakeBreak && k < len && ISPRINT(*p) && *p != shy ) {			// printable character
	    if ( *p == '&' && showprefix ) {
		cc = '&';			// assume ampersand
		if ( k < len-1 ) {
		    k++;
		...
		int hw = (k < len && *p == shy) ? shywidth : 0;
		if ( tw+sw+hw+cw > w ) {
		    if ( breakindex > 0 ) {
			breakwithinwords = FALSE;
			if ( DECCHAR(codes[breakindex]) == shy )
			    tw += shywidth;
			int c = BEGLINE | QMIN(tw,MAXWIDTH);
			if ( begline && DECCHAR(codes[begline]) == shy )
			    c |= HYPH;
			codes[begline] = c;
		...
		if ( expandtabs ) {
			...
			while ( tabindex < tabarraylen ) {
			    if ( tabarray[tabindex] > (tw+sw+cw) ) {
				ccw = tabarray[tabindex] - (tw+sw+cw);
				tabindex++;
				break;
			    }
			    tabindex++;
			...
			ccw = localTabStops - (tw+sw+cw)%localTabStops;
		    cw += ccw;
		    cc = TABSTOP | QMIN(tw+sw+cw,MAXWIDTH);
