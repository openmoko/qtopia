/****************************************************************************
** $Id: qt/src/kernel/qpixmap.cpp   2.3.12   edited 2005-10-27 $
**
...
#endif
#if defined(_WS_QWS_)
        data->copyData(pm.data->id,pm.data->rw,pm.data->d);
...
QPixmap &QPixmap::operator=( const QPixmap &pixmap )
{
	...
	if ( !isNull() ) {
#if defined(_WS_QWS_)
            pixmap.data->copyData(data->id,data->rw,data->d);
	    data->hasAlpha = pixmap.data->hasAlpha;
#else
...
void QPixmap::fill( const QWidget *widget, int xofs, int yofs )
{
    ...
    if ( bgpm ) {
	if ( !bgpm->isNull() ) {
	    QPoint ofs = qt_backgroundOffset(widget);
            xofs += ofs.x();
            yofs += ofs.y();
...
{
#if defined(_WS_QWS_) && !defined(QT_NO_SCROLLVIEW)
    // The QScrollview clipper is 4000x4000 ~ up to 64Mb pixmap.
    // Obviously impossible on an embedded device - should probably be
    // done for other platforms too.
    QScrollView *sv = 0;
    QSize olds;
    if ( w->inherits("QScrollView") ) {
	sv = (QScrollView*)w;
	if (sv->clipper() != sv->viewport()) {
	    // The clipper is enabled.  Resize to sensible size and
	    // send a dummy resize to relayout children.
	    olds = sv->viewport()->size();
	    sv->viewport()->setGeometry(0, 0, sv->clipper()->width(), sv->clipper()->height());
	    QResizeEvent re(sv->size(), sv->size());
...
#if defined(_WS_QWS_) && !defined(QT_NO_SCROLLVIEW)
    if ( sv && sv->clipper() != sv->viewport() ) {
	// The clipper is enabled.  Resize to original size and
	// send a dummy resize to relayout children.
	sv->viewport()->setGeometry(-olds.width()/2, -olds.height()/2, olds.width(), olds.height());
	QResizeEvent re(sv->size(), sv->size());
	QApplication::sendEvent(sv, &re);
    }
#endif
