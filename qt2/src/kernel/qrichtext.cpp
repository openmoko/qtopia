/****************************************************************************
** $Id: qt/src/kernel/qrichtext.cpp   2.3.12   edited 2005-10-27 $
**
...
void QTextOptions::erase( QPainter* p, const QRect& r ) const
{
    ...
    if ( p->device()->devType() == QInternal::Printer )
        return;
    bool globalBrush = FALSE;
    if ( p->device()->devType() == QInternal::Widget )
	globalBrush = ((QWidget*)p->device())->testWState(QWidget::WState_GlobalBrushOrigin);
    if ( paper->pixmap() && !globalBrush )
...
#ifdef QT_KEYPAD_MODE
	    if ( position() >= doc->selectedAnchorStartPos() && doc->selectedAnchorEndPos() >= position() )
		forceHighLight = TRUE;
#endif
            if ( to.linkUnderline ) {
                QFont f = fmt->font();
