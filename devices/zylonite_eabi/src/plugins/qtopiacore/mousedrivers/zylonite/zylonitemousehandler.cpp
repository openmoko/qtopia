...
{
    if ((mouseFD = open("/dev/input/event0", O_RDONLY | O_NDELAY)) < 0) {
      qWarning("Cannot open /dev/input/event0 (%s)", strerror(errno));
      return;
    } else
      qWarning("Opened /dev/input/event0 as touchscreen input");
    ...
    do {
      ...
      while (mouseIdx-idx >= (int)sizeof(ts_packet)) {
        ...
        if(nX >= TS_SAMPLES && nY >= TS_SAMPLES) {
          ...
          QPoint pos( (sx[index_x1] + sx[index_x2])/8,
                      (sy[index_y1] + sy[index_y2])/8);
