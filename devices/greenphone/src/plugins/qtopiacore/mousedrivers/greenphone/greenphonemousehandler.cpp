...
GreenphoneMouseHandler::GreenphoneMouseHandler()
    : m_raw(false), totX(0), totY(0), nX(0), nY(0), mouseIdx(0)
{
    qLog(Input) << "Loaded Greenphone touchscreen plugin";
    setObjectName( "Greenphone Mouse Handler" );
    openTs();
}
...
void GreenphoneMouseHandler::suspend()
{
    m_notify->setEnabled( false );
}

void GreenphoneMouseHandler::resume()
{
    m_notify->setEnabled( true );
}

void GreenphoneMouseHandler::readMouseData()
{
    if(!qt_screen)
        return;
      ...
      while (mouseIdx-idx >= (int)sizeof(ts_packet)) {
        ...
        if(data->pressure >= QT_QWS_TP_PRESSURE_THRESHOLD ) {
          ...
          if(nX >= TS_SAMPLES && nY >= TS_SAMPLES) {
            ...
            if(oldmouse.x() < 0 || oldmouse.x() > 9000 ||
               oldmouse.y() < 0 || oldmouse.y() > 9000) {
