...
while [ -e /tmp/restart-qtopia ]; do
    ...
    TIME=`date +%H:%M:%S.000`
    DATE=`date +%Y-%m-%d_%T`
    echo "Qtopia Performance Testing : Greenphone : $DATE" > /var/tmp/performance_test_$DATE.log
    echo "Performance :  runqpe :  Starting qpe... :  $TIME" >> /var/tmp/performance_test_$DATE.log
    qpe >> /var/tmp/performance_test_$DATE.log 2>&1
done
